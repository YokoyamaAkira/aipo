...
public class FeedDaemon implements Daemon {
  ...
  public static String TEMP_DIRECTORY = JetspeedResources
    .getString(JetspeedResources.TEMP_DIRECTORY_KEY);

  // akr private static boolean processed = false;

  // akr private static FeedDaemon instance = null;
  ...
  // akr private final boolean initialized = false;
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(FeedDaemon.class.getName());
  ...
  public static Feed[] getFeeds() {
    ...
    for (int i = 0; i < v.size(); ++i) {
      ...
      String description =
        JetspeedResources.getString("contentfeeds.feed."
          + name
          + ".description");

      String url =
        JetspeedResources.getString("contentfeeds.feed." + name + ".url");
    ...
    try {
      ...
      for (int i = 0; i < feeds.length; ++i) {
        ...
        try {
          ...
          // the string transformed should now contain PML... tranform it into
          // PortletMarkup
          logger.info("Determining portlets...");

          FileRegistry registry =
            (FileRegistry) TurbineServices.getInstance().getService(
  ...
  /**
    */
  // akr private void error( String message )
  // {
  ...
  public static final Reader getEntries(String url) throws Exception {
    ...
    String stylesheet =
      JetspeedResources
        .getString(JetspeedResources.CONTENTFEEDS_STYLESHEET_URL_KEY);
   ...
   */
  // akr private static String strip(String target)
  // {
