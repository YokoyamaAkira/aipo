...
public class Instantiator implements Runnable {
  ...
  /**
   * The maximum number of seconds to wait before warning that the URL took too
   * long to download
   */
  ...
  // akr private boolean forcePortet = false;

  /**
   * Static initialization of the logger for this class
   */
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(Instantiator.class.getName());
  ...
  public void getURL(String url) throws IOException {
    ...
    // if the user wants to download the URL and it isn't in the cache then go
    // for it
    if (JetspeedResources
      .getBoolean(JetspeedResources.CONTENTFEEDS_FETCHALL_KEY)
      ...
      try {
        ...
        if (total >= MAX_WARN_SECONDS) {
          logger.warn(this.getClass().getName()
            + "  The following URL took too long ("
            + total
            + " second(s)) to download: "
            + url);
        }
        ...
        logger.error("The following URL couldn't be downloaded "
          + url
          + " and took "
          + (System.currentTimeMillis() - download_begin)
          / 1000
          + " seconds to download. ");
  ...
  /**
   * Do work necessary to instantiate the current Entry but only do this if it
  ...
  public void run() {
    ...
    try {
      if (!registry.hasEntry(this.entry.getName())) {
        ...
        if (JetspeedResources
          .getBoolean(JetspeedResources.AUTOCREATE_PORTLETS_KEY)) {
      ...
      URLManager.register(this.entry.getURL(), URLManagerService.STATUS_BAD, e
        .toString());
