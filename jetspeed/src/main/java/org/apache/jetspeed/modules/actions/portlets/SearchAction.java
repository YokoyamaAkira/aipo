...
public class SearchAction extends GenericMVCAction {
   ...
   */
  @Override
  public void buildNormalContext(Portlet portlet, Context context,
      RunData rundata) throws Exception {
    List<?> results =
      (List<?>) rundata.getRequest().getAttribute(SEARCH_RESULTS);
    if (null == results) {
      results =
        (List<?>) PortletSessionState.getAttribute(rundata, SEARCH_RESULTS);
    ...
    if (searchString == null || searchString.trim().length() == 0) {
      searchString =
        (String) PortletSessionState.getAttribute(rundata, SEARCH_STRING);
    } else {
