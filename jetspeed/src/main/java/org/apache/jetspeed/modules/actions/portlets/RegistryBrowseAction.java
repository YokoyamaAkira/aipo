...
public class RegistryBrowseAction extends GenericMVCAction {
  ...
  protected void buildNormalContext(Portlet portlet, Context context,
      RunData rundata) {
    String regName =
      portlet.getPortletConfig().getInitParameter("registry", Registry.PORTLET);

    Boolean refresh =
      (Boolean) PortletSessionState.getAttribute(rundata, PREFIX
        + regName
        + ":"
    ...
    if (refresh.equals(Boolean.TRUE)) {
      ...
      PortletSessionState.clearAttribute(rundata, PREFIX
        + regName
        + ":"
        + REFRESH);
    }
    ...
    String pageSize =
      portlet.getPortletConfig().getInitParameter("page-size", "20");
    ...
    List<RegistryEntry> regEntries =
      (List<RegistryEntry>) PortletSessionState.getAttribute(
        portlet,
        rundata,
        RESULTS);
    List<RegistryEntry> filteredEntries =
      (List<RegistryEntry>) PortletSessionState.getAttribute(
        portlet,
        rundata,
        FILTERED_RESULTS);
    if (regEntries == null) {
      ...
      Collections.sort(regEntries, new Comparator<Object>() {
        @Override
  ...
  private int getStart(RunData rundata, Portlet portlet) {
    ...
    if (startInteger.intValue() == -1) {
      startInteger =
        (Integer) PortletSessionState.getAttribute(portlet, rundata, START);
  ...
  public void doFilter(RunData rundata, Context context) throws Exception {
    ...
    String regName =
      getPortlet(context).getPortletConfig().getInitParameter(
        "registry",
        Registry.PORTLET);
   ...
   * 
   * @param entries
   *          The list of registry entries to filter.
   * @param fields
   *          The array of filter names
   * @param values
   *          The array of filter values. This should be in a 1:1 ratio with the
   *          fitler names.
   * @return The list of filtered portlets.
   */
  protected List<RegistryEntry> filter(List<RegistryEntry> entries/*
                                                                   * akr
                                                                   * String[]
                                                                   * fields,
                                                                   * String[]
