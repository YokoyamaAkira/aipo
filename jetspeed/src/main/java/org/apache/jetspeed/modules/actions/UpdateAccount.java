...
/**
 * 
 * Updates an Account in the User and save the User object to backing store. You
 * must have been logged in in order to update the account.
 */
public class UpdateAccount extends Action {
  ...
  public void doPerform(RunData rundata) throws Exception {
    ...
    if (!data.getUser().hasLoggedIn()) {
      data.setScreenTemplate(JetspeedResources.getString(
        "services.JspService.screen.error.NotLoggedIn",
        "Error"));
      return;
    }
    ...
    String oldPassword =
      JetspeedSecurity.convertPassword(data.getParameters().getString(
        "old_password",
        ""));
    String password =
      JetspeedSecurity.convertPassword(data.getParameters().getString(
    ...
    if (JetspeedResources.getBoolean("automatic.logon.enable", false)) {
      if (!userRequestsRememberMe) {
        if (data.getRequest().getCookies() != null
          ...
          // remove cookies by re-adding them with zero MaxAge, which deletes
          // them
        ...
        if (data.getRequest().getCookies() == null
          || !data.getCookies().getString("username", "").equals(
            data.getUser().getUserName())
          || !data.getCookies().getString("logincookie", "").equals(
            data.getUser().getPerm("logincookie"))) {
          String loginCookieValue =
            (String) data.getUser().getPerm("logincookie");
    ...
    if (enableMail
      && (currentEmail == null || !currentEmail.equalsIgnoreCase(email))) {
    ...
    if (changepass) {
      try {
        JetspeedSecurity.changePassword(
          data.getJetspeedUser(),
          oldPassword,
          password);
  ...
  /**
   * updateUser updates the user object. Subclasses can extend this class and
   * override this method - adding additional custom settings as needed. Note
   * the default implementation does nothing - so no need to call the super
   * version.
   * 
   * @param data
  ...
  private void backToEditAccount(RunData rundata,
      Hashtable<String, String> screenData) {
