...
/**
 * This action sets up the template context for editing security group roles for
 * a given user.
 * 
 * @author <a href="mailto:morciuch@apache.org">Mark Orciuch</a>
 * @version $Id: UserGroupRoleUpdateAction.java,v 1.3 2004/03/31 04:49:10
 *          morciuch Exp $
 */
public class UserGroupRoleUpdateAction extends SecureVelocityPortletAction {
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(UserRoleUpdateAction.class.getName());
  ...
  protected void buildNormalContext(VelocityPortlet portlet, Context context,
    ...
    try {
      ...
      rundata.setMessage("Error in Jetspeed User Group Role Security: "
        + e.toString());
   ...
   * @exception Exception
   */
  private void appendNewRoleProfile(JetspeedRunData jdata, JetspeedUser user,
    ...
    if (roleProfile != null) {
      if (logger.isDebugEnabled()) {
        logger.debug("UserGroupRoleUpdateAction: retrieved profile for role: "
          + roleProfile.getPath());
      }
    }
    ...
    if (userProfile != null) {
      if (logger.isDebugEnabled()) {
        logger.debug("UserGroupRoleUpdateAction: retrieved profile for user: "
          + userProfile.getPath());
      }
    }
    ...
    if (roleProfile != null
      ...
      if (rolePortlets.getPortletsCount() > 0) {
        for (int i = 0; i < rolePortlets.getPortletsCount(); i++) {
          ...
          if (logger.isDebugEnabled()) {
            logger.debug("UserRoleUpdateAction: appended pane: "
              + pane.getId()
              + " to user: "
              + user.getUserName());
          }
  ...
  private void buildUserGroupRoleContext(VelocityPortlet portlet,
      Context context, RunData rundata, String userid) throws Exception {
    // get the user object
  ...
  public void doRoles(RunData data, Context context) throws Exception {
     ...
     * form so we will grab the hidden username and use that to retrieve the
     * user.
     */
    ...
    for (Iterator<?> groups = JetspeedSecurity.getGroups(); groups.hasNext();) {
      ...
      for (Iterator<?> roles = JetspeedSecurity.getRoles(); roles.hasNext();) {
        /*
         * In the UserRoleForm.vm we made a checkbox for every possible
         * Group/Role combination so we will compare every possible combination
         * with the values that were checked off in the form. If we have a match
         * then we will grant the user the role in the group.
         */
        Role role = (Role) roles.next();
        ...
        if (formGroupRole != null
          && JetspeedSecurity.hasRole(username, roleName, groupName) == false) {
          ...
          && JetspeedSecurity.hasRole(username, roleName, groupName)) {
