...
public abstract class VelocityControllerAction extends VelocityAction {
  ...
  /**
   * This method is used when you want to short circuit an Action and change the
   * template that will be executed next.
   * 
   * @param data
   *          Turbine information.
   * @param template
  ...
  protected Context getContext(RunData data) {
    return (Context) data.getTemplateInfo().getTemplateContext(
  ...
  public void doPerform(RunData rundata, Context context) {
    PortletController controller =
      (PortletController) context.get("controller");
  ...
  protected void buildCustomizeContext(PortletController controller,
    ...
    if (dotIdx > -1) {
      template = template.substring(0, dotIdx) + "-customize.vm";
    } else {
    ...
    // We want the save button to say different things based on whether we're
    // about to save to persistent storage
    ...
    if (((String) customizationState.getAttribute("customize-paneName"))
      .equalsIgnoreCase("*")) {
