...
/**
 * This action sets up the template context for editing security permissions in
 * the Turbine database for a given role.
 * 
 * @author <a href="mailto:taylor@apache.org">David Sean Taylor</a>
 * @version $Id: RolePermissionUpdateAction.java,v 1.9 2004/03/31 04:49:10
 *          morciuch Exp $
 */
public class RolePermissionUpdateAction extends SecureVelocityPortletAction {
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(RolePermissionUpdateAction.class.getName());
  ...
  protected void buildNormalContext(VelocityPortlet portlet, Context context,
    ...
    try {
      ...
      rundata.setMessage("Error in Jetspeed Role Permission Security: "
        + e.toString());
  ...
  /**
   * Database Update Action for Security Role Permissions. Performs updates into
   * security database.
   * 
   * @param rundata
   *          The turbine rundata context for this request.
  ...
  public void doUpdate(RunData rundata, Context context) throws Exception {
    ...
    try {
      List<?> permissions =
        (List<?>) rundata.getUser().getTemp(
          SecurityConstants.CONTEXT_PERMISSIONS);
      List<?> selected =
      ...
      for (int ix = 0; ix < permissions.size(); ix++) {
        ...
        boolean newValue =
          rundata.getParameters().getBoolean("box_" + permissionName, false);
   ...
   *          The turbine rundata context for this request.
   * @param roleName
   *          The roleName of the role that we are building a role context for.
   */
  private void buildRolePermissionContext(VelocityPortlet portlet,
      Context context, RunData rundata, String roleName)
