...
public abstract class PortletActionEvent extends ActionEvent {
  ...
  /**
   * Cache ActionEvent methods to avoid repeated replection method lookups.
   */
  private static final HashMap<String, Method> eventMethods =
  ...
  public void executeEvents(RunData data, Context context) throws Exception {
    ...
    if (!fireEvent(data, Context.class, context, theButton)
      ...
      if (!fireEvent(data, Portlet.class, portlet, theButton)) {
  ...
  protected boolean fireEvent(RunData data, Class<?> deltaClass,
    ...
    try {
      ...
      Method method = eventMethods.get(methodKey);
      if (method == null) {
