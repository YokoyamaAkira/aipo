...
public class UserBrowserAction extends VelocityPortletAction {
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(UserBrowserAction.class.getName());
  ...
  protected void buildNormalContext(VelocityPortlet portlet, Context context,
    ...
    try {
      ...
      // Currently, the getUsers(filter) is not implemented - need to do local
      // filtering
      ...
      if (filterValue != null) {
        String filterType =
          rundata.getParameters().getString(FILTER_TYPE, FILTER_TYPE_USERNAME);
        ...
        if (useRE) {
          try {
            ...
            logger
              .warn("UserBrowserAction: error processing regular expression ["
                + filterValue
                + "]: "
                + rex.toString());
          }
      ...
      if (userList.size() > numberPerPage) {
        int numberOfPages =
          (userList.size() - 1 + numberPerPage) / numberPerPage;
        int from = (currentPage - 1) * numberPerPage;
        int to = Math.min(currentPage * numberPerPage, userList.size());
        context
          .put(SecurityConstants.CONTEXT_USERS, userList.subList(from, to));

        // now build a set of links to access each page (assumed we will show
        // all links)
          ...
          if (i == currentPage) {
            ...
            if (jslink instanceof JetspeedLink) {
              pageLinks.append("[ <a href=\""
                + ((JetspeedLink) jslink)
                  .getPaneByName("UserBrowser")
                  .addQueryData(DISPLAY_PAGE, new Integer(i))
                  .toString()
                + "\">"
                + i
                + "</a> ] &nbsp;");
            } else {
              pageLinks.append("[ <a href=\"./portal/"
                + DISPLAY_PAGE
                + "/"
                + i
                + "\">"
