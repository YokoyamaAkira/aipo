...
public class UserUpdateAction extends SecureVelocityPortletAction {
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(UserUpdateAction.class.getName());
  ...
  /**
   * Build the configure state content for this portlet. TODO: we could
   * configure this portlet with configurable skins, etc..
   * 
   * @param portlet
   *          The velocity-based portlet that is being built.
   * @param context
  ...
  protected void buildNormalContext(VelocityPortlet portlet, Context context,
    ...
    try {
      ...
      if (mode != null
        && (mode.equals(SecurityConstants.PARAM_MODE_UPDATE) || mode
          .equals(SecurityConstants.PARAM_MODE_DELETE))) {
      ...
      if (msgid != null) {
        ...
        JetspeedUser tempUser =
          (JetspeedUser) rundata.getUser().getTemp(TEMP_USER);
        if (tempUser != null) {
  ...
  /**
   * Database Update Action for Users. Performs accepting of new users into
   * security database.
   * 
   * @param rundata
   *          The turbine rundata context for this request.
  ...
  public void doAccept(RunData rundata, Context context) throws Exception {
    ...
    try {
      ...
      //
      user =
        JetspeedSecurity.getUser(rundata.getParameters().getString(
      ...
      String charset =
        JetspeedResources.getString(
          "newuser.notification.charset",
          "iso-8859-1");
      se.setCharset(charset);
      Context emailContext = TurbineVelocity.getContext();
      emailContext.put("firstname", StringEscapeUtils.unescapeHtml(user
        .getFirstName()));
      emailContext.put("lastname", StringEscapeUtils.unescapeHtml(user
        .getLastName()));
      emailContext.put("username", StringEscapeUtils.unescapeHtml(user
        .getUserName()));
      ...
      String language =
        (String) user.getPerm("language", TurbineResources.getString(
          "locale.default.language",
          "en"));
      String country =
        (String) user.getPerm("country", TurbineResources.getString(
          "locale.default.country",
          "US"));
      Locale locale = new Locale(language, country);

      String templateFile =
        JetspeedResources.getString("newuser.approval.accept.template");
      String templatePath =
        TemplateLocator.locateEmailTemplate(rundata, templateFile, locale);
  ...
  public void doReject(RunData rundata, Context context) throws Exception {
    ...
    try {
      ...
      String templateFile =
        JetspeedResources.getString("newuser.approval.reject.template");
      String templatePath =
        TemplateLocator.locateEmailTemplate(rundata, templateFile, locale);
      TurbineVelocity.handleRequest(emailContext, templatePath, email_body);
    ...
    try {
      ...
      if (name == null || name.trim().length() == 0) {
        ...
        duri.addPathInfo(
          ...
          SecurityConstants.MID_INVALID_ENTITY_NAME);
        if (user != null) {
          duri.addPathInfo(SecurityConstants.PARAM_ENTITY_ID, user
            .getUserName());
        }
        duri.addQueryData(
          SecurityConstants.PARAM_MODE,
      ...
      JetspeedSecurity.forcePassword(user, rundata.getParameters().getString(
        "password"));
  ...
  public void doDelete(RunData rundata, Context context) throws Exception {
    ...
    try {
      ...
      if (rundata.getUser().getUserName().equals(user.getUserName())) {
        ...
        duri.addPathInfo(
          SecurityConstants.PARAM_MSGID,
          SecurityConstants.MID_CANT_DELETE_CURRENT);
        if (user != null) {
          duri.addPathInfo(SecurityConstants.PARAM_ENTITY_ID, user
            .getUserName());
        }
