...
/**
 * This action enables to manage psml entries within current user's security
 * context
 * 
...
public class PsmlManagerAction extends GenericMVCAction {
  ...
  private static Hashtable<String, String> queryModes =
  ...
  protected void buildConfigureContext(Portlet portlet, Context context,
    ...
    context.put(PAGE_SIZE, PortletConfigState.getParameter(
      portlet,
      rundata,
      PAGE_SIZE,
      "20"));
    setTemplate(rundata, PortletConfigState.getParameter(
      portlet,
      rundata,
      CUSTOMIZE_TEMPLATE,
      null));
  ...
  protected void buildNormalContext(Portlet portlet, Context context,
    ...
    context.put("can-search", canPerformAction(rundata, entry, entry
      .getParameter("can-search")));
    context.put("can-clone", canPerformAction(rundata, entry, entry
      .getParameter("can-clone")));
    context.put("can-import", canPerformAction(rundata, entry, entry
      .getParameter("can-import")));
    context.put("can-export", canPerformAction(rundata, entry, entry
      .getParameter("can-export")));
    context.put("can-import-all", canPerformAction(rundata, entry, entry
      .getParameter("can-import-all")));
    context.put("can-export-all", canPerformAction(rundata, entry, entry
      .getParameter("can-export-all")));
    context.put("can-add", canPerformAction(rundata, entry, entry
      .getParameter("can-add")));
    context.put("can-remove", canPerformAction(rundata, entry, entry
      .getParameter("can-remove")));

    String mode = rundata.getParameters().getString("mode", "browse");
    if (mode.equals("browse")) {
      this.setTemplate(rundata, PortletConfigState.getParameter(
        portlet,
        rundata,
        "template",
        "browser-psml"));
   ...
   * @param rundata
   */
  private void buildBrowseNormalContext(Portlet portlet, Context context,
      RunData rundata) {
    ...
    String pageSize =
      PortletConfigState.getParameter(portlet, rundata, PAGE_SIZE, "20");
    ...
    String defaultSearch =
      PortletConfigState.getParameter(portlet, rundata, DEFAULT_SEARCH, "true");

    // Get the iterator
    DatabaseBrowserIterator windowIterator =
      (DatabaseBrowserIterator) PortletSessionState.getAttribute(
        portlet,
        rundata,
        PROFILE_ITERATOR);
    if (refreshFlag) {
      ...
      Integer type =
        (Integer) PortletSessionState.getAttribute(
          portlet,
          rundata,
          LAST_SEARCH_TYPE);
      String value =
        (String) PortletSessionState.getAttribute(
          portlet,
          rundata,
          LAST_SEARCH_VALUE);
      ...
      if (type != null && value != null) {
        windowIterator =
          performSearch(rundata, portlet, type.intValue(), value);
      }
      ...
      windowIterator =
        performSearch(rundata, portlet, QueryLocator.QUERY_USER, rundata
          .getUser()
          .getUserName());
    }
   ...
   * @return
   */
  private String canPerformAction(RunData rundata, PortletEntry entry,
    ...
    if (actionParam != null && entry != null) {
      ...
      result =
        new Boolean(JetspeedSecurity.checkPermission((JetspeedUser) rundata
          .getUser(), portalResource, JetspeedSecurity.PERMISSION_CUSTOMIZE))
          .toString();

      // System.out.println("parameter=" + actionParam.getName() +
      // ", canAccess = " + result.toString());
    }
  ...
  public void doFilter(RunData rundata, Context context) {
    ...
    int filterType =
      rundata.getParameters().getInt(FILTER_TYPE, QueryLocator.QUERY_ALL);
   ...
   * @return
   */
  private DatabaseBrowserIterator performSearch(RunData rundata,
      Portlet portlet, int type, String value) {
    ...
    int size =
      Integer.parseInt(PortletConfigState.getParameter(
        portlet,
        rundata,
        PAGE_SIZE,
        "20"));
    DatabaseBrowserIterator windowIterator =
      new DatabaseBrowserIterator(entries, entryType, entryType, size);
    PortletSessionState.clearAttribute(portlet, rundata, PROFILE_ITERATOR);
    ...
    PortletSessionState.setAttribute(
      ...
      new Integer(type));
    PortletSessionState
      .setAttribute(portlet, rundata, LAST_SEARCH_VALUE, value);
   ...
   * 
   * @param portlet
   *          The template-based portlet that is being built.
   * @param context
   *          The velocity context for this request.
   * @param rundata
   *          The turbine rundata context for this request.
   */
  private void buildDetailNormalContext(Portlet portlet, Context context,
      RunData rundata) {
    try {
      //
      ...
      if (mode != null && mode.equals(SecurityConstants.PARAM_MODE_INSERT)) {
        ...
        context.put("defMediaType", jdata
          .getCapability()
        ...
        if (msgid == null) {
          ...
          context
            .put(CATEGORY_VALUE, rundata.getUser().getTemp(CATEGORY_VALUE));
        ...
        if (msgid == null) {
          String tmpPath =
            JetspeedResources.getString(
              JetspeedResources.TEMP_DIRECTORY_KEY,
              "/tmp");
          String exportPath =
            JetspeedResources.getString(
              "psml.export.default.path",
              TurbineServlet.getRealPath(tmpPath));
          if (path == null) {
            context.put(COPY_TO, exportPath);
            context.put(COPY_FROM, Profiler.PARAM_USER
              + File.separator
              + Profiler.PARAM_ANON
              + File.separator
      ...
      if (mode != null && mode.equals("import")) {
        ...
        if (msgid == null) {
          ...
          if (path == null) {
            ...
            context
              .put("copyFrom", root + File.separator + filePath.toString());
          }
        } else {
          context.put(CATEGORY_NAME, rundata.getUser().getTemp(CATEGORY_NAME));
          context
      ...
      rundata.setScreenTemplate(JetspeedResources.getString(
        "template.error",
        "Error"));
    }
  }
  ...
  public void doInsert(RunData rundata, Context context) throws Exception {
    ...
    try {
      ...
      if (PsmlManager.getDocument(locator) != null) {
        throw new EntityExistsException("Profile:"
          + locator.getPath()
          + " Already Exists!");
      }
      ...
      if (copyFrom.equals("none")) {
        ...
        // profile.getDocument().getPortlets().getTitle() + " for " +
        ...
        if (doc != null) {
          ...
          Portlets clonedPortlets =
            (Portlets) SerializationUtils.clone(portlets);
  ...
  public void doDelete(RunData rundata, Context context) throws Exception {
    try {
      ...
      DynamicURI duri =
        link.addPathInfo(
          SecurityConstants.PARAM_MODE,
          SecurityConstants.PARAM_MODE_DELETE).addPathInfo(
          SecurityConstants.PARAM_MSGID,
    ...
    try {
      ...
      if (baseProfile != null) {
        ...
        if (doc != null) {
          ...
          rundata.addMessage("Profile ["
            + copyFrom
            + "] has been saved to disk in ["
            + copyTo
            + "]<br>");
        }
      } else {
      ...
      JetspeedLink link = JetspeedLinkFactory.getInstance(rundata);
      DynamicURI duri =
        link.addPathInfo(SecurityConstants.PARAM_MODE, "export").addPathInfo(
          SecurityConstants.PARAM_MSGID,
          SecurityConstants.MID_UPDATE_FAILED);
      JetspeedLinkFactory.putInstance(link);
      rundata.setRedirectURI(duri.toString());
  ...
  public void doExportall(RunData rundata, Context context) throws Exception {
    ...
    try {
      ...
      while (i.hasNext()) {
        ...
        if (doc != null) {
          ...
          if (!this.saveDocument(copyToFile.toString(), doc)) {
            logger.error("Failed to save PSML document for ["
              + profile.getPath());
          } else {
            String msg =
              "Profile ["
      ...
      DynamicURI duri =
        link
          .addPathInfo(SecurityConstants.PARAM_MODE, "export_all")
          .addPathInfo(
            SecurityConstants.PARAM_MSGID,
            SecurityConstants.MID_UPDATE_FAILED);
      JetspeedLinkFactory.putInstance(link);
  ...
  private boolean saveDocument(String fileOrUrl, PSMLDocument doc) {
    ...
    try {
      ...
      logger.error("PsmlManagerAction: Could not marshal the file "
        + f.getAbsolutePath(), e);
      ...
      logger.error("PsmlManagerAction: Could not save the file "
        + f.getAbsolutePath(), e);
    } catch (Exception e) {
      logger.error("PsmlManagerAction: Error while saving  "
        + f.getAbsolutePath(), e);
    } finally {
      try {
        writer.close();
  ...
  public void doImport(RunData rundata, Context context) throws Exception {
    ...
    try {
      ...
      if (name == null || name.trim().length() == 0) {
        JetspeedLink link = JetspeedLinkFactory.getInstance(rundata);
        DynamicURI duri =
          link.addPathInfo(SecurityConstants.PARAM_MODE, "import").addPathInfo(
            SecurityConstants.PARAM_MSGID,
            SecurityConstants.MID_INVALID_ENTITY_NAME);
        JetspeedLinkFactory.putInstance(link);
        rundata.setRedirectURI(duri.toString());
      ...
      if (doc != null) {
        Portlets portlets = doc.getPortlets();
        //
        // Profiler does not provide update capability - must remove before
        // replacing
        //
        if (PsmlManager.getDocument(locator) != null) {
          Profiler.removeProfile(locator);
      ...
      rundata.addMessage("Profile for ["
        + locator.getPath()
        + "] has been imported from file ["
        + copyFrom
        + "]<br>");
      setRefreshPsmlFlag(rundata, TRUE);
  ...
  public void doImportall(RunData rundata, Context context) throws Exception {
    ...
    try {
      ...
      DynamicURI duri =
        link
          .addPathInfo(SecurityConstants.PARAM_MODE, "import_all")
          .addPathInfo(
            SecurityConstants.PARAM_MSGID,
            SecurityConstants.MID_UPDATE_FAILED);
      JetspeedLinkFactory.putInstance(link);
   ...
   * 
   * @param path
   *          The formatted profiler path string.
   * @param path
   *          fully qualified .psml file name
  ...
  private ProfileLocator mapFileToLocator(String path) throws Exception {
    ...
    if (tokens.size() > 0) {
      result.setName(tokens.lastElement());
    }
  ...
  private String mapLocatorToFile(ProfileLocator locator) {
    ...
    if (null != locator.getName()) {
      if (!(locator.getName().endsWith(Profiler.DEFAULT_EXTENSION))) {
        path.append(File.separator).append(locator.getName()).append(
          Profiler.DEFAULT_EXTENSION);
      } else {
