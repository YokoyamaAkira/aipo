...
/**
 * This action validates the form input from the NewAccount Screen. If it is
 * valid, then it will check to make sure that the user account does not already
 * exist. If it does, then it will show the NewAccount screen again. If it
 * doesn't alread exist, then it will create the new user and set the
 * CONFIRM_VALUE to be the users session id. This part should probably be
 * re-done to get a better less hackable CONFIRM_VALUE, but this should work for
 * now. If everything goes well, this action will send the user a confirmation
 * email and then show the ConfirmRegistration screen.
 * 
 * @author Jon S. Stevens <a href="mailto:jon@clearink.com">jon@clearink.com</a>
 * @author David S. Taylor <a
 *         href="mailto:david@bluesunrise.com">david@bluesunrise.com</a>
...
public class CreateNewUserAndConfirm extends Action {
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(CreateNewUserAndConfirm.class.getName());
  ...
  public void doPerform(RunData data) throws Exception {
    try {
      ...
      if (str != null
        && str.equalsIgnoreCase(data.getParameters().getString(submit, ""))) {
      ...
      String CHNAME =
        Localization.getString(data, "CREATENEWUSERANDCONFIRM_DUPLICATEMSG");
  ...
  /**
   * createUser creates a new user. Subclasses can override this method - adding
   * additional user property settings as needed. The default implementation
   * does nothing.
   * 
   * @param user
   *          the new user that has been created
   * @param data
   *          the current RunData instance
   * 
   * @throws Exception
   *           passed up from JetspeedSecurity
   */
   ...
   * email The new user is logged on and then redirected to the home page
   * 
   * @param data
   *          Turbine information.
   * @param username
