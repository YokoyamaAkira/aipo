...
/**
 * This action enables to browse any of the psml info, for displaying available
 * entries and information on these entries
 * 
...
public class PsmlBrowseAction extends VelocityPortletAction {
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(PsmlBrowseAction.class.getName());
  ...
  protected void buildNormalContext(VelocityPortlet portlet, Context context,
    ...
    boolean refreshFlag =
      (rundata.getUser().getTemp(PSML_REFRESH_FLAG, FALSE)).equals(TRUE);
    ...
    if ((windowIterator == null) || refreshFlag) {
      ...
      if (filterValue != null && !filterValue.trim().equalsIgnoreCase("")) {
        String filterType =
          rundata.getParameters().getString(FILTER_TYPE, FILTER_TYPE_USER);
        ...
        if (useRE) {
          try {
            ...
            logger
              .warn("PsmlBrowseAction: error processing regular expression ["
                + filterValue
                + "]: "
                + rex.toString());
          }
