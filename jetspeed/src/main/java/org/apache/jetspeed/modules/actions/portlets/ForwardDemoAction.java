...
public class ForwardDemoAction extends VelocityPortletAction {
  ...
  protected void buildNormalContext(VelocityPortlet portlet, Context context,
      RunData rundata) {
    String next =
      (String) PortletSessionState.getAttribute(rundata, PARAM_NEXT);
    if (null == next) {
      next =
        PortletConfigState
          .getParameter(portlet, rundata, PARAM_NEXT, "NOT_SET");
    ...
    List<?> forwards =
      (List<?>) PortletSessionState.getAttribute(rundata, VAR_FORWARDS);
    if (null == forwards) {
    ...
    List<?> portletForwards =
      (List<?>) PortletSessionState.getAttribute(rundata, VAR_TARGETS);
