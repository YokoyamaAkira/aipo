...
/**
 * The purpose of this class is to allow one to load and execute Parameter
 * modules.
 * 
...
public class ParameterLoader extends GenericLoader {
   ...
   */
  private static ParameterLoader instance = new ParameterLoader(
    JetspeedResources.getInt("parameter.cache.size", 50));

  /**
   * Static initialization of the logger for this class
   */
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(ParameterLoader.class.getName());

  /**
   * These ctor's are private to force clients to use getInstance() to access
   * this class.
   */
  ...
  /**
   * Attempts to load and render a parameter using custom style. For example,
   * one might define a custom parameter presentation style TextArea which
   * displays current value of the parameter using HTML text area presentation.
   * Assuming that TextArea is rendered using two optional parameters: rows and
   * cols, the map passed to this method could contain the following values: <li>
   * symbols.style.rows = 5 <li>symbols.style.cols = 80 and the call might look
   * like this:
   * <p>
   * String symbols = eval(data, "TextArea", "symbols", "MSFT,SUNW,EMC,ORCL",
   * parms);
   * 
   * @param data
   *          Turbine information.
   * @param provider
   *          Custom parameter class name (without the package)
   * @param name
   *          Name for rendered HTML tag
   * @param value
   *          Current value
   * @param parms
   *          Optional rendition parameters
   * @return
   * @exception Exception
   *              a generic exception.
   */
  public String eval(RunData data, String provider, String name, String value,
    ...
    Map<String, String> styleparms = extractStyleParameters(parms, name);
  ...
  /**
   * Pulls out an instance of the object by name. Name is just the single name
   * of the object.
   * 
   * @param provider
   ...
   *              a generic exception.
   */
  public ParameterPresentationStyle getInstance(String provider)
      throws Exception {
    ...
    if (cache() && this.containsKey(provider)) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug("ParameterLoader: Serving parameter: "
          + provider
          + ", prm="
          + prm
          + " from cache");
      ...
      AssemblerBrokerService ab =
        (AssemblerBrokerService) TurbineServices.getInstance().getService(
          AssemblerBrokerService.SERVICE_NAME);

      try {
        // Attempt to load the presentation style
        prm =
          (ParameterPresentationStyle) ab.getAssembler("parameter", provider);
        if (prm == null) {
          if (logger.isDebugEnabled()) {
          ...
          ab.registerFactory(
            "parameter",
            new ParameterPresentationStyleFactory());
          prm =
            (ParameterPresentationStyle) ab.getAssembler("parameter", provider);
        }
        if (logger.isDebugEnabled()) {
          logger.debug("ParameterLoader: Loaded parameter: "
            + provider
            + ", prm="
            + prm);
        }
      ...
      if (prm == null) {
        ...
        // FIX ME: The AssemblerFactories should each add it's own string
        // here...
   ...
   */
  public static Map<String, String> extractStyleParameters(
      Map<String, String> parms, String parmName) {
      ...
      while (it.hasNext()) {
        ...
        if (parmkey.startsWith(key)) {
          try {
            ...
            if (logger.isDebugEnabled()) {
              logger.debug("ParameterLoader: parm name ["
                + parmName
                + "] - storing option ["
                + stylekey
                + "] with value ["
