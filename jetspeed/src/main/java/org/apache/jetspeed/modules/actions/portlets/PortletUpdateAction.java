...
public class PortletUpdateAction extends RegistryUpdateAction {
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(PortletUpdateAction.class.getName());
  ...
  protected void buildNormalContext(VelocityPortlet portlet, Context context,
    ...
    if (mode != null && mode.equals(SecurityConstants.PARAM_MODE_INSERT)) {
      context.put("parents", PortletFilter.buildParentList(PortletFilter
        .getAllPortlets()));
      context.put("securitys", CustomizeSetAction.buildList(
        rundata,
        Registry.SECURITY));
    }
    ...
    if (mode != null && mode.equals(SecurityConstants.PARAM_MODE_UPDATE)) {
      ...
      context.put("categories", PortletFilter.buildCategoryList(PortletFilter
        .getAllPortlets()));
      ...
      context.put("media_types", iteratorToCollection(mediaTypeReg
        .listEntryNames()));

      if (portletEntry.getType() != null
        && portletEntry.getType().equals("ref")) {
        PortletEntry parentEntry =
          (PortletEntry) Registry.getEntry(Registry.PORTLET, portletEntry
            .getParent());
        if (parentEntry == null) {
          logger
          ...
          Collection<?> parentCategories =
            iteratorToCollection(parentEntry.listCategories());
          context.put("parent_categories", parentCategories);

          Collection<?> parentMediaTypes =
            iteratorToCollection(parentEntry.listMediaTypes());
      ...
      if (portletEntry.getType().equals("ref")) {
        ...
        if (parentEntry == null) {
          ...
          while (paramIter.hasNext()) {
            ...
            BaseParameter param =
              (BaseParameter) portletEntry.getParameter(paramName);
            BaseParameter clonedParameter =
              (BaseParameter) SerializationUtils.clone(param);
  ...
  /**
   * @see org.apache.jetspeed.modules.actions.portlets.RegistryUpdateAction#updateRegistryEntry(org.apache.turbine.util.RunData,
      ...
      if (portletEntry != null) {
        ...
        if (categoryName != null && categoryName.length() > 0) {
          String categoryGroup =
            rundata.getParameters().getString("category_group", "Jetspeed");
        ...
        logger
          .error("Failed to find registry entry while trying to add category");
      }
    } catch (Exception e) {
      DynamicURI duri =
        redirect(
  ...
  public void doRemovecategories(RunData rundata, Context context)
    ...
    try {
      ...
      if (portletEntry != null) {
        ...
        logger
          .error("Failed to find registry entry while trying to remove categories");
      }
    } catch (Exception e) {
      DynamicURI duri =
        redirect(
  ...
  /**
   * @see org.apache.jetspeed.modules.actions.portlets.RegistryUpdateAction#redirect(org.apache.turbine.util.RunData,
