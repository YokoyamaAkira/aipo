...
public class SecurityUpdateAction extends RegistryUpdateAction {
  ...
  /**
   * @see org.apache.jetspeed.modules.actions.portlets.VelocityPortletAction#buildNormalContext(VelocityPortlet,
   *      Context, RunData)
   */
  @Override
  ...
  public void doAddaccess(RunData rundata, Context context) throws Exception {
    try {
      ...
      if (securityEntry != null) {
        ...
        logger
          .error("Failed to find registry entry while trying to add accesses");
      }
    } catch (Exception e) {
      DynamicURI duri =
        redirect(
  ...
  public void doRemoveallow(RunData rundata, Context context) throws Exception {
    try {
      ...
      if (securityEntry != null) {
          ...
          if (allowIndexes != null && allowIndexes.length > 0) {
            for (int i = 0; i < allowIndexes.length; i++) {
              ...
              if (allowIndex >= 0) {
                ...
                logger
                  .error("Allow Index: " + allowIndex + " is out of range.");
              }
            }
          } else {
            DynamicURI duri =
        ...
        logger
          .error("Failed to find registry entry while trying to remove allow");
      }
    } catch (Exception e) {
      DynamicURI duri =
        redirect(
