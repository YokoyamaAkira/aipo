...
/**
 * This action sets up the template context for retrieving paged data from the
 * resultSet according to the quey speciified by the user.
 * 
...
public class DatabaseBrowserAction extends VelocityPortletAction implements
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(DatabaseBrowserAction.class.getName());

  /**
   * Build the maximized state content for this portlet. (Same as normal state).
   * 
   * @param portlet
   *          The velocity-based portlet that is being built.
   * @param context
  ...
  protected void buildConfigureContext(VelocityPortlet portlet,
    ...
    context.put(WINDOW_SIZE, getParameterUsingFallback(
      portlet,
      rundata,
      WINDOW_SIZE,
      null));
    setTemplate(rundata, getParameterUsingFallback(
      portlet,
      rundata,
      CUSTOMIZE_TEMPLATE,
      null));
  ...
  protected void buildNormalContext(VelocityPortlet portlet, Context context,
    ...
    String sortColName =
      getRequestParameter(portlet, rundata, SORT_COLUMN_NAME);
    int start =
      getStartVariable(portlet, rundata, START, sortColName, iterator);

    windowSize =
      Integer.parseInt(getParameterUsingFallback(
        portlet,
        rundata,
        WINDOW_SIZE,
        "10"));
    ...
    try {
      if (iterator == null
        || PortletSessionState.getPortletConfigChanged(portlet, rundata)) {
      ...
      if (iterator != null) {
         ...
         * System
         * .out.println("buildNormalContext Titles= "+iterator.getResultSetTitleList
         * ());
         * System.out.println("buildNormalContext windowSize="+windowSize+" prev="
         * +prev+
   ...
   * 
   * @see
   * org.apache.jetspeed.modules.actions.portlets.browser.BrowserQuery#filter
   * (java.util.List, RunData)
   */
  @Override
  public boolean filter(List<Object> row, RunData rundata) {
    return false;
  }
  ...
  protected void getRows(VelocityPortlet portlet, RunData rundata, String sql,
    ...
    try {
      ...
      /*
       * get the user object types to be displayed and add them to the title
       * list as well as the result set list
       */
      List<?> userObjList =
        (List<?>) getParameterFromTemp(portlet, rundata, USER_OBJECTS);
      ...
      /*
       * the array columnDisplayed maintains a boolean value for each column
       * index. Only the columns that are set to true are added to the
       * resultSetList, resultSetTitleList and resultSetTypeList.
       */
      boolean[] columnDisplayed = new boolean[columnNum + userObjListSize];

      /*
       * this for loop constructs the columnDisplayed array as well as adds to
       * the resultSetTitleList and resultSetTypeList
       */
      ...
      for (int i = columnNum; i < columnNum + userObjListSize; i++) {
        ActionParameter usrObj =
          (ActionParameter) userObjList.get(i - columnNum);
        resultSetTitleList.add(usrObj.getName());
        resultSetTypeList.add(usrObj.getType());
        columnDisplayed[i] = true;
        // System.out.println("User List Name = "+
        // usrObj.getName()+" Type = "+usrObj.getType());
      }
      /*
  ...
  /**
   * Centralizes the calls to runData.getUser.getTemp() - to retrieve the
  ...
  protected BrowserIterator getDatabaseBrowserIterator(VelocityPortlet portlet,
    ...
    BrowserIterator iterator =
      (BrowserIterator) getParameterFromTemp(
        portlet,
        rundata,
        DATABASE_BROWSER_ACTION_KEY);
    return iterator;
  ...
  /**
   * Centralizes the calls to runData.getUser.setTemp() - to set the
   * DatabaseBrowserIterator.
   * 
   * @param data
   *          The turbine rundata context for this request.
  ...
  protected void setDatabaseBrowserIterator(VelocityPortlet portlet,
      RunData rundata, BrowserIterator iterator) {
    setParameterToTemp(portlet, rundata, DATABASE_BROWSER_ACTION_KEY, iterator);
  }

  /**
   * Centralizes the calls to runData.getUser.removeTemp() - to clear the
   * DatabaseBrowserIterator from the temp storage.
   * 
   * @param data
   *          The turbine rundata context for this request.
   * 
   ...
   * returns null, then it gets the value from the psml file. If the psml value
   * is null then it returns the value from the xreg file.
   * 
   */
  protected String getQueryString(VelocityPortlet portlet, RunData rundata,
      Context context) {
  ...
  /**
     *
     */
  protected String getParameterUsingFallback(VelocityPortlet portlet,
      RunData rundata, String attrName, String attrDefValue) {
    return PortletConfigState.getParameter(
      portlet,
      rundata,
      attrName,
      attrDefValue);
  ...
  protected void clearParameterFromPSML(VelocityPortlet portlet,
      RunData rundata, String attributeName) {
  ...
  /**
     *
     */
  protected String getParameterFromPSML(VelocityPortlet portlet,
      RunData rundata, String attrName, String attrDefValue) {
  ...
  protected String getParameterFromRegistry(VelocityPortlet portlet,
    ...
    return PortletConfigState.getConfigParameter(
      portlet,
      attrName,
      attrDefValue);
  ...
  /**
     *
     */
  protected Object getParameterFromTemp(VelocityPortlet portlet,
      RunData rundata, String attrName) {
  ...
  /**
     *
     */
  protected String getRequestParameter(VelocityPortlet portlet,
      RunData rundata, String attrName) {
    if (isMyRequest(portlet, rundata)) {
  ...
  protected int getStartVariable(VelocityPortlet portlet, RunData rundata,
      String attrName, String sortColName, BrowserIterator iterator) {
    int start = -1;
  ...
  protected void readSqlParameters(VelocityPortlet portlet, RunData rundata) {
    ...
    while (true) {
      String param =
        getParameterUsingFallback(portlet, rundata, SQL_PARAM_PREFIX + i, null);
      if (param == null) {
        break;
      } else {
  ...
  protected void readUserParameters(VelocityPortlet portlet, RunData rundata,
      Context context) {
    ...
    if (userObjRead != null) {
      ...
      String userObjNames =
        getParameterFromRegistry(portlet, USER_OBJECT_NAMES, null);
      if (userObjTypes != null && userObjTypes.length() > 0) {
        ...
        while (tokenizer1.hasMoreTokens() && tokenizer3.hasMoreTokens()) {
          userObjectList.add(userObjectIndex, new ActionParameter(tokenizer1
            .nextToken(), null, tokenizer3.nextToken()));
          userObjectIndex++;
        }
  ...
  protected void readLinkParameters(VelocityPortlet portlet, RunData rundata,
      Context context) {
    ...
    if (linksRead != null && ((String) linksRead).equals(LINKS_READ)) {
      ...
      String rowLinkClasses =
        getParameterFromRegistry(portlet, ROW_LINK_TARGETS, null);
      String rowLinkTypes =
        getParameterFromRegistry(portlet, ROW_LINK_TYPES, null);
      if (rowLinkIds != null && rowLinkIds.length() > 0) {
        ...
        while (tokenizer1.hasMoreTokens()
          ...
          rowList.add(rowIndex, new ActionParameter(
      ...
      String tableLinkIds =
        getParameterFromRegistry(portlet, TABLE_LINK_IDS, null);
      String tableLinkClasses =
        getParameterFromRegistry(portlet, TABLE_LINK_TARGETS, null);
      String tableLinkTypes =
        getParameterFromRegistry(portlet, TABLE_LINK_TYPES, null);
      if (tableLinkIds != null && tableLinkIds.length() > 0) {
        ...
        while (tokenizer1.hasMoreTokens()
          && tokenizer2.hasMoreTokens()
          && tokenizer3.hasMoreTokens()) {
          tableList.add(tableIndex, new ActionParameter(
            tokenizer1.nextToken(),
            tokenizer2.nextToken(),
            tokenizer3.nextToken()));
          tableIndex++;
   ...
   * wanted to track the parent of an object based on some calculation per row,
   * it could be done here.
   * 
   */
  public void populate(int rowIndex, int columnIndex, List<Object> row) {
  }
