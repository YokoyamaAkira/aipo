...
public interface BrowserQuery {
  ...
  /**
   * Filter the row programmatically on a query. By returning true, instruct the
   * database browser to filter the row. By returning false, instruct the
   * database browser to keep the row. (Filtering means removing the row from
  ...
  /*
   * Set a list of JDBC query parameters. All members of this list must be java
   * objects (not primitives) Should be called from derived classes.
   */
