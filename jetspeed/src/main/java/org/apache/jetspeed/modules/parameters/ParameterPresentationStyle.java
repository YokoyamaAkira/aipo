 ...
 * @version $Id: ParameterPresentationStyle.java,v 1.4 2004/02/23 03:01:20 jford
 *          Exp $
 */
public abstract class ParameterPresentationStyle extends Assembler {
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(ParameterPresentationStyle.class.getName());

  private Map<String, String> styleparms = null;
   ...
   *          run context info
   * @param name
   *          name for the returned control
   * @param value
   *          default value for the control
   * @param parms
   *          hashtable with presentation parameters
   * @return html for the control
   */
  public abstract String getContent(RunData data, String name, String value,
  ...
  /**
   * <P>
   * Returns javascript event definitions as defined by
   * "[name].style.javascript:[event]".
   * </P>
   * <P>
   * For example: <CODE>symbols.style.javascipt:onChange</CODE>
   * 
   * @return map of javascript events
   */
  public Map<String, String> getJavascriptEvents() {

    Hashtable<String, String> result = null;

    if (this.styleparms != null) {
      Iterator<String> it = this.styleparms.keySet().iterator();
      while (it.hasNext()) {
