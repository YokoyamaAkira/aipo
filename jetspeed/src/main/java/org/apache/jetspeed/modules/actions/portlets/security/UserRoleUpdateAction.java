 ...
 * Turbine database for a given user.
 * 
 * @author <a href="mailto:taylor@apache.org">David Sean Taylor</a>
 * @version $Id: UserRoleUpdateAction.java,v 1.12 2004/03/31 04:49:10 morciuch
 *          Exp $
 */
public class UserRoleUpdateAction extends SecureVelocityPortletAction {
  ...
  protected void buildNormalContext(VelocityPortlet portlet, Context context,
    ...
    try {
      ...
      rundata.setMessage("Error in Jetspeed User Role Security: "
        + e.toString());
  ...
  public void doUpdate(RunData rundata, Context context) throws Exception {
    ...
    try {
      List<?> roles =
        (List<?>) rundata.getUser().getTemp(SecurityConstants.CONTEXT_ROLES);
      List<?> selected =
      ...
      for (int ix = 0; ix < roles.size(); ix++) {
        boolean newValue =
          rundata.getParameters().getBoolean(
            "box_" + ((Role) roles.get(ix)).getName(),
            false);
        ...
        if (newValue != oldValue) {
          if (newValue == true) {
            ...
            // If role profile merging is active, append profile for the new
            // role
            ...
            JetspeedSecurity.revokeRole(user.getUserName(), ((Role) roles
              .get(ix)).getName());
          }
        }
      }
  ...
  private void appendNewRoleProfile(JetspeedRunData jdata, JetspeedUser user,
    ...
    if (roleProfile != null) {
      if (logger.isDebugEnabled()) {
        logger.debug("UserRoleUpdateAction: retrieved profile for role: "
          + roleProfile.getPath());
      }
    }
    ...
    if (userProfile != null) {
      if (logger.isDebugEnabled()) {
        logger.debug("UserRoleUpdateAction: retrieved profile for user: "
          + userProfile.getPath());
      }
    }
   ...
   */
  private void buildUserRoleContext(VelocityPortlet portlet, Context context,
      RunData rundata, String userid) throws Exception {
    // get the user object
