 ...
 * <LI><code>layout</code> [$northsouth|<strong>$eastwest</strong>]:
 * presentation layout</LI>
...
public class CheckBoxGroup extends ParameterPresentationStyle {
  ...
  public String getContent(RunData data, String name, String value,
    ...
    String items = parms.get(ITEMS);
    String layout = parms.get(LAYOUT);
    String prefix = parms.get(PREFIX);
    if (prefix == null) {
  ...
  private String getJavascript(String name, Vector<String> v, String prefix) {
    ...
    for (Enumeration<String> e = v.elements(); e.hasMoreElements();) {
      String item = prefix + e.nextElement();
      result.append("((");
      result.append(item);
  ...
  public static void main(String args[]) {
    ...
    java.util.Hashtable<String, String> parms = new Hashtable<String, String>();
    parms
      .put(
        ITEMS,
        "Tomaszewski,Gorgon,Zmuda,Szymanowski,Musial,Kasperczak,Deyna,Cmikiewicz,Lato,Szarmach,Gadocha");
    System.out.println(cbg.getContent(
      null,
      "test",
      "Deyna,,,,Gorgon,Lato,Szarmach,",
      parms));
