...
/**
 * An abstract base class with default actions for many of the common fields and
 * parameters shared by the registry entries. To add a new registry update
 * action, simply derive from this class and override the resetForm,
 * clearUserData, and updateRegistry functions. If you need to provide more
 * actions that those that are provided, simply create them in your derived
 * class.
 * 
 * @author <a href="mailto:caius1440@hotmail.com">Jeremy Ford</a>
 * @version $Id: RegistryUpdateAction.java,v 1.10 2004/03/31 04:49:10 morciuch
 *          Exp $
 */
public abstract class RegistryUpdateAction extends SecureVelocityPortletAction {
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(RegistryUpdateAction.class.getName());
  ...
  protected void buildNormalContext(VelocityPortlet portlet, Context context,
    ...
    if (msgid != null) {
      ...
      if (id < SecurityConstants.MESSAGES.length) {
        context
          .put(SecurityConstants.PARAM_MSG, SecurityConstants.MESSAGES[id]);
      }
    }
    ...
    try {
      ...
      if (entryName == null || entryName.length() == 0) {
        ...
        if (existingEntry != null) {
          throw new EntityExistsException("RegistryEntry: "
            + entryName
            + " Already Exists!");
        }
        ...
        rundata.getUser().setTemp(
          RegistryBrowseAction.PREFIX
            + registry
            + ":"
            + RegistryBrowseAction.REFRESH,
          Boolean.TRUE);
      }
  ...
  /**
   * Basic implementation of a method to update a registry entry. The fields
   * that are common to all registry entries can simply be added below.
   * 
   * @param rundata
   *          The turbine rundata context for this request.
   * @param registryEntry
   ...
   */
  protected void updateRegistryEntry(RunData rundata,
    ...
    if (hasChanged(String.valueOf(registryEntry.isHidden()), String
      .valueOf(hidden))) {
        ...
        if (parameterName != null && parameterName.length() > 0) {
          ...
          if (regEntry instanceof PortletEntry) {
            ...
            boolean isCachedOnName =
              rundata.getParameters().getBoolean("cached_on_name", false);
            boolean isCachedOnValue =
              rundata.getParameters().getBoolean("cached_on_value", false);
          ...
          for (int i = 0; i < parameters.length; i++) {
            ...
            if (regEntry instanceof PortletEntry) {
              ...
              ((BaseCachedParameter) parameter)
                .setCachedOnValue(isCachedOnValue);
            ...
            String description =
              rundata.getParameters().getString(parameterName + ".description");
            String title =
              rundata.getParameters().getString(parameterName + ".title");
            ...
            String securityRef =
              rundata
                .getParameters()
                .getString(parameterName + ".security_ref");
            String type =
              rundata.getParameters().getString(parameterName + ".type");
            boolean isHidden =
              rundata.getParameters().getBoolean(
  ...
  /**
   * Method that sets up a redirect link given the rundata, the mode, and a
   * reason
   * 
   * @param rundata
   * @param mode
   ...
   */
  protected Collection<Object> iteratorToCollection(Iterator<?> iter) {
