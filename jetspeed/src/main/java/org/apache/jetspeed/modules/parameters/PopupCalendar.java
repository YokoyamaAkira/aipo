 ...
 * Calendar is accessed by clicking the icon next to input field. For now, the
 * only date format supported is mm/dd/yyyy. If value <code>$today</code> is
 * passed, the date in the input field will default to today's date.
 * <p>
 * Options:
 * <UL>
 ...
 * default=mm/dd/yyyy. The date format can have three types of separators:
 * hyphen(-), space( ), or slash(/), but must be consistent in their usage. E.g.
 * d/m/yyyy The acceptable tokens are :
 * <UL>
 * <LI>d - day</LI>
...
public class PopupCalendar extends ParameterPresentationStyle {
   ...
   */
  @Override
  public String getContent(RunData data, String name, String value,
    ...
    container.addElement(new Script().setLanguage("JavaScript").setSrc(
      "javascript/PopupCalendar.js"));

    container.addElement(new Input(Input.TEXT, name, value));

    IMG img =
      new IMG("images/cal.gif")
        .setAlt("Click here for popup calendar")
        .setBorder(0);
  ...
  private String getJavaScript(String fieldName) {
    ...
    String formName =
      (String) this.getParm(PARM_FORM_NAME, "DefaultCustomizer");
    String format = (String) this.getParm(PARM_FORMAT, "mm/dd/yyyy");

    Object[] args = { formName, fieldName, format };

    String template =
      "javascript: show_calendar(''{0}.{1}'',{0}.{1}.value,''{2}'');";
