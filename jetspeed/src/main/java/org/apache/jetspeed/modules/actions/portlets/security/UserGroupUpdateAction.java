 ...
 * @version $Id: UserGroupUpdateAction.java,v 1.5 2004/03/31 04:49:10 morciuch
 *          Exp $
 */
public class UserGroupUpdateAction extends SecureVelocityPortletAction {
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(UserGroupUpdateAction.class.getName());
      ...
      for (int ix = 0; ix < groups.size(); ix++) {
        boolean newValue =
          rundata.getParameters().getBoolean(
            "box_" + ((Group) groups.get(ix)).getName(),
            false);
        boolean oldValue = ((Boolean) selected.get(ix + 1)).booleanValue();
        if (newValue != oldValue) {
          if (newValue == true) {
            // grant a role to a user
            JetspeedSecurity.joinGroup(user.getUserName(), ((Group) groups
              .get(ix)).getName());
          } else {
            // revoke a role from a user
            JetspeedSecurity.unjoinGroup(user.getUserName(), ((Group) groups
              .get(ix)).getName());
          }
        }
      }
   ...
   */
  private void buildUserGroupsContext(VelocityPortlet portlet, Context context,
