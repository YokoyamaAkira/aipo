...
public class ClientUpdateAction extends RegistryUpdateAction {
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(ClientUpdateAction.class.getName());
    ...
    try {
      ...
      if (clientEntry != null) {
        ...
        logger.error(this.getClass().getName()
          + ": Failed to find registry entry while trying to add mime type");
      }
    } catch (Exception e) {
      DynamicURI duri =
        redirect(
      ...
      if (clientEntry != null) {
        ...
        logger
          .error(this.getClass().getName()
            + ": Failed to find registry entry while trying to remove mime types");
      }
    } catch (Exception e) {
      DynamicURI duri =
        redirect(
   ...
   *          The velocity context for this request.
   */
  private void updateClientEntry(RunData rundata, ClientEntry clientEntry)
      throws Exception {
    ...
    if (hasChanged(clientEntry.getUseragentpattern(), userAgentPattern)) {
      try {
        ...
        logger.error(whoAmI
          + "Illegal regular expression syntax "
          + userAgentPattern
          + " for user agent");
        logger.debug(whoAmI
          + "Illegal regular expression syntax for user agent", e);

        throw new IllegalArgumentException(
          "Illegal regular expression syntax for user agent");
      }
    }
