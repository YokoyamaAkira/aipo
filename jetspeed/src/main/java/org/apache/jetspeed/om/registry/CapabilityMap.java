...
/**
 * <P>
 * The <CODE>CapabilityMap</CODE> interface represents a list that stores
 * capabilities a client is capable of. It is accessed by the portlet container
 * to get information about the client's capabilities.
 * </P>
 * <P>
 * The names of the capabilities are matched by the class variable names of the
 * class <CODE>Capability</CODE>. To add a capability use the class variable
 * name and <B>not</B> the internally used string value.
 * </P>
 * 
...
public interface CapabilityMap {
  ...
  /**
   * Checks if the all the elements of argument capability map are included in
   * the current one
   * 
   * @param map
