...
/**
 * Base simple bean-like implementation of the ConfigElement interface suitable
 * for Castor XML serialization.
 * 
...
public/* abstract */class PsmlConfigElement implements ConfigElement,
  ...
  public String getParameterValue(String name) {
    ...
    for (int ix = 0; ix < parameters.size(); ix++) {
      Parameter param = parameters.elementAt(ix);
  ...
  public Parameter[] getParameter() {
    ...
    for (int index = 0; index < size; index++) {
      mArray[index] = parameters.elementAt(index);
    }
    return mArray;
  ...
  public Object clone() throws java.lang.CloneNotSupportedException {
    ...
    if (this.parameters != null) {
      ((PsmlConfigElement) cloned).parameters =
        new Vector<Parameter>(this.parameters.size());
      Iterator<Parameter> it = this.parameters.iterator();
      while (it.hasNext()) {
        ((PsmlConfigElement) cloned).parameters.add((Parameter) it
          .next()
          .clone());
      }
    }
