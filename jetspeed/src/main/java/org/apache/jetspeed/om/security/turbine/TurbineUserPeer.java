 ...
 */
public class TurbineUserPeer extends
   ...
   * JetspeedUsers, and populating each user's hashtable of attributes from the
   ...
   */
  public static List<JetspeedUser> doSelectUsers(Criteria criteria)
   ...
   */
  public static JetspeedUser row2UserObject(Record row, int offset, Class<?> cls)
      throws TorqueException {
    try {
  ...
  /*
   * Populates a JetspeedUser from a single row in the result set. The hashtable
   * of attributes for each user is populated with columns from the result set.
   * 
   * @param row a row containing a single user's attributes.
   * 
   * @param user the JetspeedUser object to be populated.
   */
  protected static void populateUserObject(Record row, int offset,
      JetspeedUser user) throws Exception {
    ...
    Hashtable<String, Object> tempHash =
      (Hashtable<String, Object>) ObjectUtils.deserialize(objectData);
    if (tempHash == null) {
   ...
   */
  /*
   * DST: COMMITTED 8/19 FROM AMIT AMTE - IT HAS SOME BUGS, GOING TO REMOVE
   * UNTIL RESOLVED
   * 
   * protected static void populateUserObject(Record row, int offset,
   * JetspeedUser user) throws Exception { // Restore the Permanent Storage
   * Hashtable. First the // Hashtable is restored, then any explicit table
   * columns // which should be included in the Hashtable are added. byte[]
   * objectData = (byte[])row.getValue(OBJECTDATA).asBytes(); Hashtable tempHash
   * = (Hashtable)ObjectUtils.deserialize(objectData); if (tempHash == null) {
   * tempHash = new Hashtable(10); }
   * 
   * for( int j=0; j<columnNames.length; j++ ) { if (!
   * columnNames[j].equalsIgnoreCase( OBJECTDATA ) ) { Object obj2 = null; Value
   * value = null; try { value = row.getValue(columnNames[j]); } catch
   * (Exception ex) { // Column missing from the torque record .. //
   * Probable/intentional mis-match between the // torque schema and db table
   * continue; }
   * 
   * if (value.isString()) obj2 = value.asString(); else if
   * (value.isBigDecimal()) obj2 = value.asBigDecimal(); else if
   * (value.isBytes()) obj2 = value.asBytes(); else if (value.isDate()) obj2 =
   * value.asDate(); else if (value.isShort()) obj2 = new
   * Short(value.asShort()); else if (value.isInt()) obj2 = new
   * Integer(value.asInt()); else if (value.isLong()) obj2 = new
   * Long(value.asLong()); else if (value.isDouble()) obj2 = new
   * Double(value.asDouble()); else if (value.isFloat()) obj2 = new
   * Float(value.asFloat()); else if (value.isBoolean()) obj2 = new
   * Boolean(value.asBoolean()); else if (value.isTime()) obj2 = value.asTime();
   * else if (value.isTimestamp()) obj2 = value.asTimestamp(); else if
   * (value.isUtilDate()) obj2 = value.asUtilDate(); else if (value.isByte())
   * obj2 = new Byte(value.asByte());
   * 
   * if ( obj2 != null ) { if (columnNames[j].equalsIgnoreCase(
   * JetspeedUser.USER_ID )) { obj2 = value.toString(); }
   * 
   * System.out.println(columnNames[j] + ": " + obj2.toString()); tempHash.put(
   * columnNames[j], obj2 ); } } } user.setPermStorage( tempHash ); }
   */
  ...
  public static Criteria buildCriteria(JetspeedUser user) {
    ...
    for (int i = 1; i < columnNames.length; i++) {
      if (permData.containsKey(columnNames[i])) {
        String key =
          new StringBuffer(TABLE_NAME)
            .append('.')
            .append(columnNames[i])
            .toString();

        criteria.add(key, permData.remove(TurbineUserPeer.columnNames[i]));
      }
    }
    String dataKey =
      new StringBuffer(TABLE_NAME)
        .append('.')
        .append(TURBINE_OBJECTDATA)
        .toString();
