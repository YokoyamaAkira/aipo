...
/**
 * PortletIterator - seamless iterator over nested vectors of portlet
 * collections
 *
 *
...
public class PortletIterator implements Iterator<Object> {
  ...
  protected BasePortletEntry getParentEntry(BasePortletEntry entry) {
    ...
    parent =
      (BasePortletEntry) Registry.getEntry(Registry.PORTLET, entry.getParent());
    return parent;
  }
  ...
  protected Vector<?> getVector() {
    try {
      this.vector =
        (Vector<?>) this.entry
          .getClass()
          .getMethod(this.method, null)
          .invoke(this.entry, null);
    } catch (Exception e) {
      this.vector = null;
    }
