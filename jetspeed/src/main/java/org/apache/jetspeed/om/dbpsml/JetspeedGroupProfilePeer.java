 ...
 * @version $Id: JetspeedGroupProfilePeer.java,v 1.13 2003/08/13 04:18:17 taylor
 *          Exp $
 */
public class JetspeedGroupProfilePeer extends
    org.apache.jetspeed.om.dbpsml.BaseJetspeedGroupProfilePeer implements
    DBOperations {
   ...
   * 
   * @param profile
   *          Profile object that will be inserted in the database
   * @param connection
   ...
   * 
   * @param profile
   *          Profile object that will be deleted from the database
   * @param connection
   *          A database connection to use
   */
  ...
  private void doInsertOrUpdate(Profile profile, int operation,
    ...
    DatabasePsmlManager service =
      (DatabasePsmlManager) PsmlManager.getService();
    ...
    groupProfile.setProfile(DBUtils.portletsToBytes(profile
      .getDocument()
      .getPortlets(), service.getMapping()));
   ...
   *          A database connection to use
   */
  public void delete(ProfileLocator locator, Connection connection)
      throws Exception {
    super.doDelete(buildCriteria(locator), connection);
  }

  /**
   * Select group profile record from the database table for the given locator
   * object.
   * 
   * @param locator
   *          ProfileLocator object that will be used to select required profile
   *          from the database
   * @param connection
   *          A database connection to use
   * @return List of records that statisfy the given locator criteria.
   */
  public List<JetspeedGroupProfile> select(ProfileLocator locator,
      Connection connection) throws Exception {
  ...
  /**
   * Select group profile record from the database table for the given locator
   * object and return list ordered by primary key..
   * 
   * @param locator
   *          ProfileLocator object that will be used to select required profile
   *          from the database
   ...
   * @return List of records that statisfy the given locator criteria.
   */
  public List<JetspeedGroupProfile> selectOrdered(ProfileLocator locator,
      Connection connection) throws Exception {
   ...
   * 
   * @param group
   *          Group object for which all the records will be deleted from the
   *          database
   * @param connection
   *          A database connection to use
   */
