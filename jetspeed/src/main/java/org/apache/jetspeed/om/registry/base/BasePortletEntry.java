...
/**
 * Default bean like implementation of the PortletEntry interface suitable for
 * serialization with Castor
 *
 * @author <a href="mailto:raphael@apache.org">Rapha Luta</a>
 * @version $Id: BasePortletEntry.java,v 1.5 2004/02/23 03:08:26 jford Exp $
 */
public class BasePortletEntry extends BasePortletInfoEntry implements
    PortletEntry, java.io.Serializable {
  ...
  /**
   * This method is used by the Castor persistence system to retrieve the
   * application status
   *
  ...
  public Map<String, String> getParameterMap() {
    Hashtable<String, String> params =
      (Hashtable<String, String>) super.getParameterMap();

    if (isRef) {
      Map<String, String> map = getParentEntry().getParameterMap();
      map.putAll(params);
      return map;
    }
  ...
  /**
   * Test if a given media type is supported by this entry. The test is done by
  ...
  public void removeCategory(String name, String group) {
    for (int ix = 0; ix < categories.size(); ix++) {
      Category cat = categories.elementAt(ix);
      if (cat.getName().equals(name) && cat.getGroup().equals(group)) {
