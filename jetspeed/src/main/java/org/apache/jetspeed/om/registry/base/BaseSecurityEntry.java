 ...
 */
public class BaseSecurityEntry extends BaseRegistryEntry implements
   ...
   */
  @Override
  public boolean allowsSpecificGroupRole(String action, String group,
      String role) {
    SecurityAccess access = getAccess(action);
    if (access.getAllAllows() != null) {
      Iterator<?> allAllows = access.getAllows().iterator();
   ...
   *          Map of allow-if
   * @param mapType
   *          ROLE_MAP or USER_MAP or GROUP_MAP or GROUP_ROLE_MAP
   * @param mapKey
   *          role or user to test
   * @param allKey
