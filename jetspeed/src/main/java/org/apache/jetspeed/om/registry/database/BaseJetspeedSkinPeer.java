 ...
 * @version $Id: BaseJetspeedSkinPeer.java,v 1.3 2004/04/06 23:00:16 morciuch
 *          Exp $
 */
public class BaseJetspeedSkinPeer extends BasePeer implements DBRegistry {
  ...
  protected static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(BaseJetspeedSkinPeer.class.getName());
   ...
   *           into a TorqueException.
   */
  public static void populateObject(Record row, int offset, BaseSkinEntry obj)
      throws TorqueException {
    try {
      ...
      BaseMetaInfo baseMetaInfo =
        new BaseMetaInfo(row.getValue(offset + 4).asString(), row.getValue(
          offset + 5).asString(), row.getValue(offset + 6).asString());
   ...
   *           into a TorqueException.
   */
  public static List<BaseSkinEntry> doSelect(Criteria criteria)
      throws TorqueException {
   ...
   *           into a TorqueException.
   */
  public static List<BaseSkinEntry> populateObjects(List<?> records)
      throws TorqueException {
    List<BaseSkinEntry> results = new ArrayList<BaseSkinEntry>(records.size());
    // populate the object(s)
    for (int i = 0; i < records.size(); i++) {
      Record row = (Record) records.get(i);
      results.add(BaseJetspeedSkinPeer.row2Object(row, 1, BaseJetspeedSkinPeer
        .getOMClass()));
    }
    return results;
  }
  ...
  public static void buildSkinParameters(int id, BaseSkinEntry obj)
    ...
    try {
      List<?> list =
        BaseJetspeedSkinParameterPeer.retrieveById(SimpleKey.keyFor(id));
      if (list != null) {
