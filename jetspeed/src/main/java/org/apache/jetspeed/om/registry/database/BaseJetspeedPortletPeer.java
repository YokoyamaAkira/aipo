 ...
 * @version $Id: BaseJetspeedPortletPeer.java,v 1.3 2004/04/06 23:00:16 morciuch
 *          Exp $
 */
public class BaseJetspeedPortletPeer extends BasePeer implements DBRegistry {
  ...
  protected static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(BaseJetspeedPortletPeer.class.getName());
   ...
   */
  public static List<BasePortletEntry> resultSet2Objects(
   ...
   *           into a TorqueException.
   */
  public static BasePortletEntry row2Object(Record row, int offset, Class<?> cls)
      throws TorqueException {
    try {
   ...
   *           into a TorqueException.
   */
  public static void populateObject(Record row, int offset, BasePortletEntry obj)
      throws TorqueException {
    try {
      ...
      BaseMetaInfo baseMetaInfo =
        new BaseMetaInfo(row.getValue(offset + 10).asString(), row.getValue(
          offset + 11).asString(), row.getValue(offset + 12).asString());
   ...
   *           into a TorqueException.
   */
  public static List<BasePortletEntry> doSelect(Criteria criteria)
      throws TorqueException {
   ...
   *           into a TorqueException.
   */
  public static List<BasePortletEntry> populateObjects(List<?> records)
      throws TorqueException {
    List<BasePortletEntry> results =
      new ArrayList<BasePortletEntry>(records.size());
    // populate the object(s)
    for (int i = 0; i < records.size(); i++) {
      Record row = (Record) records.get(i);
      results.add(BaseJetspeedPortletPeer.row2Object(
        row,
        1,
  ...
  public static void buildPortletParameters(int id, BasePortletEntry obj)
    ...
    try {
      List<?> list =
        BaseJetspeedPortletParameterPeer.retrieveById(SimpleKey.keyFor(id));
  ...
  public static void buildPortletMedia(int id, BasePortletEntry obj)
    ...
    try {
      List<?> list =
        BaseJetspeedPortletMediaTypePeer.retrieveById(SimpleKey.keyFor(id));
      for (int i = 0; i < list.size(); i++) {
  ...
  public static void buildPortletCategory(int id, BasePortletEntry obj)
    ...
    try {
      List<?> list =
        BaseJetspeedPortletCategoryPeer.retrieveById(SimpleKey.keyFor(id));
      for (int i = 0; i < list.size(); i++) {
        ...
        } else if (!obj.hasCategory(name, group)) {
