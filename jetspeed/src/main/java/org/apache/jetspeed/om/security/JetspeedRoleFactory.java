...
/**
 * Factory class for creating Jetspeed Roles. The role class is configured in
 * the JR.p
 * 
 */
public class JetspeedRoleFactory {
  ...
  public static Role getInstance(boolean isNew) throws RoleException {
    ...
    if (null == roleClassName) {
      try {
        ...
        throw new RoleException(
          "RoleFactory: Failed to create a Class object for Role implementation: "
            + e.toString());
      }
    }
    ...
    try {
      ...
      throw new RoleException(
        "Failed instantiate an Role implementation object: " + e.toString());
    }
