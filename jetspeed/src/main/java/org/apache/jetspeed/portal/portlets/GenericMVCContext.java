 ...
 * context created by this object always has priority over an addtional chained
 * objects. In short matching keys within the initial context will "hide"
 * matching keys within the addtional context(s).
 * </p>
 * 
 * @author tkuebler
 ...
 * Note:
 * 
 * create the generic context interface later just use Velocity's for now
 */
public class GenericMVCContext implements Context {

  private final HashMap<String, Object> data;

  private final HashSet<Context> additionalContexts;
  ...
  /**
   * Adds an existing Collection of contexts into this one. Externally added
   * contexts are maintained indvidually and are not merged into the existing
   * context. Done to facilitate the context chanining.
   * 
   * @author <a href="mailto:sweaver@rippe.com">Scott Weaver</a>
   */
  ...
  /**
   * Adds an existing context into this one. Externally added contexts are
   * maintained indvidually and are not merged into the existing context Done to
   * facilitate the context chanining.
   * 
   * @author <a href="mailto:sweaver@rippe.com">Scott Weaver</a>
   */
  ...
  public boolean containsKey(java.lang.Object key) {
    ...
    if (!found) {
      ...
      while (itr.hasNext() && !found) {
        found = itr.next().containsKey(key);
      }
    }
  ...
  public Object[] getKeys() {
    ...
    while (itr.hasNext()) {
      Object[] keys = itr.next().getKeys();
      for (int i = 0; i < keys.length; i++) {
        keySet.add((String) keys[i]);
      }
    }
   ...
   * 
   * @param Context
   *          context Additional Context object to add.
   * @author <a href="mailto:sweaver@rippe.com">Scott Weaver</a>
   */
  ...
  /**
   * This Collection is "live" as it is the same Collection that maintains this
   * Context's chained contexts. This Collection DOES NOT include objects
   * maintained in the initial context.
   * 
