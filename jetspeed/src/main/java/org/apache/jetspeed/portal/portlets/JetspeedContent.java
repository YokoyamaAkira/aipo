...
public class JetspeedContent extends FileWatchPortlet {
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(JetspeedContent.class.getName());
  ...
  public void init() throws PortletException {
    ...
    this.url =
      JetspeedResources.getString("content.provider." + provider + ".url");

    this.stylesheet =
      JetspeedResources.getString("content.provider."
        + provider
        + ".stylesheet.url");
    ...
    if (url == null
      ...
      throw new PortletException(
        "Not all properties defined in JetspeedResources.  See JetspeedResources.properties notes.");
    }
  ...
  private ConcreteElement parse(String url) throws PortletException {
    ...
    try {
      ...
      this.stylesheet =
        JetspeedDiskCache.getInstance().getEntry(this.stylesheet).getURL();
    } catch (IOException e) {
      logger.error("Couldn't transform content.", e);
      throw new PortletException(
        "Couldn't transform content.  Please see error log");
    }

    try {

      return new JetspeedClearElement(SimpleTransform
        .transform(url, stylesheet));
