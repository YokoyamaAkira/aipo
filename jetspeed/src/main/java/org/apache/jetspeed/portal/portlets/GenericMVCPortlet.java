...
/**
 * Provides the basic MVC Portlet functionality independant of any specific view
 * technology (ie jsp, velocity, xslt, etc). It handles the views via a
 * ViewProcessor, which is a pluggable, factory created, run time module for
 * which ever view technology your portlet uses.
 * 
 * There is no need to extend this portlet class, just define your porlet entry
 * in the registy as a child of this class and provide your template and action
 * class (extened from GenericMVCAction of course) and you are good to go.
 * 
 * Example .xreg entry:
 * 
 * <portlet-entry name="GenericMVCPortlet" hidden="false" type="abstract"
 * application="false"> <security-ref parent="default"/>
 * <classname>com.cisco.it.psf.portal.portlets.GenericMVCPortlet</classname>
 * <media-type ref="html"/> <url cachedOnURL="true"/> </portlet-entry>
 * <portlet-entry name="VelocityMVCExample" hidden="false" type="ref"
 * parent="GenericMVCPortlet" application="false"> <meta-info> <title>Velocity
 * MVC Portlet</title> <description>Velocity Generic MVC Portlet</description>
 * </meta-info>
 * <classname>com.cisco.it.psf.portal.portlets.GenericMVCPortlet</classname>
 * <parameter name="template" value="mvc-example" hidden="true"
 * cachedOnName="true" cachedOnValue="true"/> <parameter name="viewtype"
 * value="Velocity" hidden="true" cachedOnName="true" cachedOnValue="true"/>
 * <parameter name="action" value="portlets.ExampleGenericMVCAction"
 * hidden="true" cachedOnName="true" cachedOnValue="true"/> <url
 * cachedOnURL="true"/> </portlet-entry>
 * 
 * See the Velocity and JSP MVC Portlet examples for template and action class
 * clues.
 * 
 * To add new view processor types, simply implement the ViewProcessor interface
 * and add your type into the <b>viewprocessor.properties</b> file as shown in
 * the example below:
 * 
...
public class GenericMVCPortlet extends AbstractInstancePortlet {
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(GenericMVCPortlet.class.getName());
  ...
  public void init() throws PortletException {
    ...
    logger.info("GenericMVCPortlet - creating view processor for viewtype = "
      + viewType
      + ", template = "
      + template);
  ...
  public boolean isCacheable() {
    return getPortletConfig()
      .getInitParameter(IS_CACHEABLE, "true")
      .equalsIgnoreCase("true");
  }

  /**
   * Whether or not this portlet provides it's own customizer. This can be set
   * at the registry level by adding a boolean paramter "provides.customization"
   * Defaults to "false"
   */
  @Override
  ...
  public ConcreteElement getContent(RunData rundata) {
    if (useDelayedRendering(rundata)) {
      ...
      String docUrl =
        "portal/" + path + "/js_peid/" + portletId + "?action=controls.Print";
    ...
    // is the cached content still valid, if not, generate and return the
    // content
  ...
  protected ConcreteElement buildContent(RunData rundata) {
    ...
    GenericMVCContext context =
      new GenericMVCContext(TurbinePull.getGlobalContext());
    ...
    if (actionName != null) {
      ...
      try {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug("GenericMVCPortlet: Executing action ["
            + actionName
            + "] for portlet ["
            + this.getName()
            + "]");
  ...
  /**
   * STW: Added for backward compatibility when using this class to subclass the
   * existing Jsp and Velocity portlets so they can set their view prior to
   * super.init();
   * 
   * @param viewType
   *          The viewType to set
  ...
  /**
   * This is called before any action execution happens. This provides backward
   * compatibility to JspPortletActions who retreive information, like Portlet,
   * from the request BEFORE the ViewProcessor.processView() is called which
   * normally populates the request with Context objects.
   * 
  ...
  protected String getCurrentTemplate(RunData data) {
    String useTemplate =
      (String) PortletSessionState.getAttribute(this, data, TEMPLATE);
    if (useTemplate == null) {
  ...
  protected boolean useDelayedRendering(RunData rundata) {
    ...
    return renderingDelayed
      && (action == null || action.length() == 0 || "controls.Restore"
        .equals(action));
  }

  protected ConcreteElement renderJspTemplate(RunData rundata,
      String templateName) {
