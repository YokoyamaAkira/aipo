 ...
 * to be used in any other place except the PortletServlet as it is just
 * displays information about another servlet. If in the future this should be
 * used elsewhere it can as it is a 1st class Servlet.
 * </p>
 * 
 * <p>
...
public class PortletInfoPortlet extends AbstractPortlet {
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(PortletInfoPortlet.class.getName());

  public static final String THIRDPARTY_PORTLETRENDERER_URL_KEY =
    "thirdparty.portletrenderer.url";

  public static final String THIRDPARTY_PORTLETRENDERER_CAPTION_KEY =
    "thirdparty.portletrenderer.caption";
  ...
  public ConcreteElement getContent(RunData rundata) {
    ...
    if (url != null) {
      ...
      try {
        long urlUpdated =
          JetspeedDiskCache.getInstance().getEntry(url).getLastModified();
        t.addElement(this.getRow("URL last updated:  "
          + df.format(new Date(urlUpdated))));
        long urlExpires =
          JetspeedDiskCache.getInstance().getEntry(url).getExpirationTime();
        t.addElement(this.getRow("URL expires:  "
          + df.format(new Date(urlExpires))));
      } catch (IOException e) {
        logger.error("Exception", e);
      }
    }

    t.addElement(this.getRow("Portlet last updated:  "
      + df.format(new Date(portlet.getCreationTime()))));

    // BEGIN 3RD PARTY REPL

    t.addElement(new TR().addElement(new TD().addElement(new B()
      .addElement("Actions:"))));
    ...
    try {
      ...
      // FIX ME: If the portlet is viewed in Avantgo and then portlet info is
      // restored, the portlet will
      // be maximized (similar to customizing certain portlet types. The desired
      // effect would be to
      // set the portlet's mode to normal.
      internal =
        jsLink
          .addPathInfo("js_peid", js_peid)
          .setAction("controls.Maximize")
          .toString();
    } catch (Exception e) {
      logger.error("Exception", e);
    }
    ...
    StringBuffer external =
      new StringBuffer(getPortletConfig().getInitParameter(
        THIRDPARTY_PORTLETRENDERER_URL_KEY));
    ...
    t.addElement(new TR().addElement(new TD().addElement(new A(external
      .toString()).setTarget("_blank").addElement(message))));
     ...
     * t.addElement( new TR().addElement( new TD() .addElement( new
     * B().addElement( "Mime Types:" ) ) ) );
     * 
     * MimeType[] mts =
     * portlet.getPortletConfig().getCapabilityMap().getMimeTypes();
     * 
     * for ( int i = 0; i < mts.length; ++i ) {
     * 
     * t.addElement( new TR() .addElement( new TD( mts[i].toString() ) ) );
     * 
     * }
     */
    ...
    while (names.hasNext()) {
      ...
      t.addElement(new TR().addElement(new TD(name + ":  " + value)));
  ...
  private ConcreteElement getRow(String message) {
    ...
    return new TR().addElement(new TD().setNoWrap(true).addElement(message));
