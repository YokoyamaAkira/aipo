...
/**
 * <p>
 * This control renders the title of a portlet for MimeTyps WML and HTML. For
 * WML only the title (represented as a link) is returned. In case that the
 * device requests html the title will be rendered within a titlebar (with
...
public class TitleControl extends AbstractPortletControl {
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(TitleControl.class.getName());
  ...
  public ConcreteElement getHTMLContent(RunData rundata) {
    ...
    TR finalTitle =
      new TR().setBgColor(this.getTitleColor()).addElement(
        new TD().setBgColor(this.getTitleColor()).setNoWrap(true).setWidth(
          "100%").setVAlign("middle").addElement(
          new B().addElement(getPortlet().getTitle()).addElement("  ")));
  ...
  public ConcreteElement getWMLContent(RunData rundata) {
    ...
    try {
      ec.addElement(new org.apache.ecs.wml.P()
        .addElement(new org.apache.ecs.wml.A(URILookup.getURI(
          URILookup.TYPE_HOME,
          URILookup.SUBTYPE_MAXIMIZE,
          getName(),
          rundata)).addElement(getTitle())));
  ...
  /**
   * Method returns the title of the portlet, which is placed within this
   * control.
   * 
   ...
   * Moreover, it checks, if the included portlet fits the given MimeTyp as
   * well. Thus, the method returns true, iff both the control and the
   * portlet(set) support the requested MimeType. Otherwise false is returned.
   * 
   * @param mimeType
          ...
          if (type != URILookup.TYPE_INFO) {
            try {
              org.apache.ecs.html.A edit =
                new org.apache.ecs.html.A(URILookup.getURI(
                  URILookup.TYPE_INFO,
                  URILookup.SUBTYPE_MARK,
                  this.getPortlet(),
                  rundata)).addElement(new IMG(content
                  .getURI(JetspeedResources.INFO_IMAGE)).setBorder(0));
          ...
          if (subtype != URILookup.SUBTYPE_MAXIMIZE) {
            org.apache.ecs.html.A max =
              new org.apache.ecs.html.A(URILookup.getURI(
                ...
                this.getPortlet(),
                rundata)).addElement(new IMG(content
                .getURI(JetspeedResources.MAX_IMAGE)).setBorder(0));
