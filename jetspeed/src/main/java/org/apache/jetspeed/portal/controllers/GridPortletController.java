 ...
 * <li><b>columnWidths</b> optional, the size of the columns, separated by a
 * colon</li>
 * <li><b>rows<b> optional, number of rows of the grid. If not specified,
 * determined by the layout information in each portlet</li>
 * <li><b>rowHeights</b> optional, the minimum size of the rows, separated by a
 * colon</li>
 * </ul>
 * 
 * </p>
 * <p>
 * The controller expects each portlet to have a row number and a column number
...
public class GridPortletController extends AbstractPortletController {
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(GridPortletController.class.getName());
  ...
  public ConcreteElement getContent(RunData rundata) {
    ...
    try {
      ...
      for (int i = 0; i < rows; ++i) {
        ...
        for (int j = 0; j < cols; ++j) {
          row.addElement(td =
            new TD().setVAlign("top").addElement(elements[i][j]));
          if (getRowHeight(i) != null) {
            td.setHeight(getRowHeight(i));
          }
          if (getColumnWidth(j) != null) {
            td.setWidth(getColumnWidth(j));
          }
        }
  ...
  public String getColumnWidth(int pos) {
    if (pos < colWidths.size()) {
      return colWidths.elementAt(pos);
    }
    return null;
  }
  ...
  public String getRowHeight(int pos) {
    if (pos < rowHeights.size()) {
      return rowHeights.elementAt(pos);
    }
    return null;
  }
