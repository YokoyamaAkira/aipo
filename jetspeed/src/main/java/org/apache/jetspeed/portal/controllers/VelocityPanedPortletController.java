...
/**
 * A Velocity based portlet controller implementation that can be used to manage
 * paned content (ie, where a only a subset of all portlets is visible at any
 * given time)
 * 
 * @author <a href="mailto:raphael@apache.org">Rapha�l Luta</a>
 * @author <a href="mailto:taylor@apache.org">David Sean Taylor</a>
 * 
 * @version $Id: VelocityPanedPortletController.java,v 1.13 2004/02/23 03:25:06
 *          jford Exp $
 */
...
public class VelocityPanedPortletController extends VelocityPortletController
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(VelocityPanedPortletController.class.getName());
  ...
  public boolean isSelected(Portlet p, RunData rundata) {
    ...
    String pname =
      rundata.getParameters().getString(JetspeedResources.PATH_PANENAME_KEY);
    ...
    if (peid == null) {
      if (last == null) {
        ...
        if (pname == null) {
          ...
          // If the current portlet set has a portlet with the same name as the
          // one we're trying to select
          // we don't want to select anything else b/c we'll select the one we
          // want AND the last-used portlet.
          // If the portlet set doesn't have a portlet by this name, we WANT to
          // select the last
          // used, otherwise nothing will be selected in this set.
          return (getPortlets().getPortletByName(pname) == null && p
            .getID()
            .equals(last));
        }
      }
      ...
      if (subPane != null && p.getID().equals(subPane)) {
        ...
        if (!p.getAttribute("_menustate", "open", rundata).equals("closed")) {
          SessionState state =
            ((JetspeedRunData) rundata).getPortletSessionState(getPortlets()
              .getID());
  ...
  /**
   * Returns the pane id from the session for pane selection of this portlet set
   * / portal page
   * 
   * @param rundata
   *          The request data.
   ...
   * for this portlet set / portal page.
   * 
   * @param rundata
   *          The request data.
   * @param id
   *          The tab id to save for this controller
   */
  @Override
  public void savePaneID(RunData data, String id) {
    // get the state for this portlet (portlet set) in this page in this session
    SessionState state =
      ((JetspeedRunData) data).getPortletSessionState(getPortlets().getID());
  ...
  /**
   * Sets the name of the parameter that will define which pane should be
   * displayed
   * 
   * @deprecated
   * 
