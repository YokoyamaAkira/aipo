 ...
 * PERFORMANCE NOTE:
 * 
 * getContent returns a StringElement that was generated on setContent(). This
 * is used so that performance is increased since ECS does not have to work
 * overtime to generate output.
 * </p>
 * 
 ...
 */
public abstract class AbstractPortlet implements Portlet, PortletState,
    Cacheable, Refreshable {

  /**
   * Static initialization of the logger for this class
   */
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(AbstractPortlet.class.getName());
  ...
  /**
   * Holds instances of ConcreteElements (Portlet output/content) based on its
   * current CapabilityMap.
   */
  protected Hashtable<String, ConcreteElement> content =
    new Hashtable<String, ConcreteElement>();
  ...
  protected void setContent(String content) {
    this.setContent(new JetspeedClearElement(content), CapabilityMapFactory
      .getDefaultCapabilityMap());
  }

  /**
    */
  protected void setContent(ConcreteElement content, CapabilityMap map)
  ...
  /**
   * Usually called by caching system when portlet is marked as expired, but has
   * not be idle longer then TimeToLive.
   * 
  ...
  public void refresh() {
    /*
     * The following message is here to add in debugging. It is expected the any
     * portlet type that is refreshable will implement this method.
     */
  ...
  /**
   * Is this portlet cacheable. It is the portlet's responsability to cache the
   * content.
   * 
   ...
   * 
   * @param cacheable
   *          <CODE>true</CODE> Portlet is cachable<BR>
  ...
  /**
   * <p>
   * Used by the cache to get a unique reference on what you want to add and
   * then retrieve in the future from the cache
   * </p>
   * 
   * <p>
   * Most implementations should just call the CacheHandleManager with the given
   * params within the implementation and just return this.
   * </p>
   * 
  ...
  /**
   * Sets the cache expiration time. When the portlet is stale (expired), the
   * refresh() will be called if the portlet has not been untouched longer then
   * then it's TimeToLive.
   * 
   * @param expirationMillis
   *          setExpirationMillis Expiration in milliseconds since epoch
   */
  @Override
  public void setExpirationMillis(long expirationMillis) {
    ...
    if (cachedObject != null) {
      long expirationInterval =
        this.expirationMillis.longValue() - cachedObject.getCreated();
   ...
   * 
   * @param config
   *          The configuration object to use for building the handle
   * 
   ...
   * 
   * @param cachedObject
   *          Cached Object associated to this portlet
   */
  @Override
  ...
  public ConcreteElement getContent(RunData rundata, CapabilityMap map) {
    ...
    return content.get(mymap.toString());
  }
  ...
  public ConcreteElement getContent(RunData rundata, CapabilityMap map,
    ...
    ConcreteElement element = content.get(mymap.toString());
    ...
    if (element == null) {
      if (allowRecurse) {
        try {
          ...
          element =
            new JetspeedClearElement("Error when retrieving Portlet contents");
        ...
        if (element == null) {
          ...
          mymap = CapabilityMapFactory.getDefaultCapabilityMap();
          element = content.get(mymap.toString());
          if (element == null) {
            element =
              new JetspeedClearElement("Unknown Problem getting Contents");
          }
        }
      }
    }
   ...
   * @param rundata
   *          The RunData object for the current request
   * @return <CODE>true</CODE> Editing is allow <CODE>false</CODE> Editing is
   *         NOT alowed
   */
  @Override
   ...
   * @param rundata
   *          The RunData object for the current request
   * @return <CODE>true</CODE> Viewing is allow <CODE>false</CODE> Viewing is
   *         NOT alowed
   * 
  ...
  public boolean supportsType(MimeType mimeType) {
    PortletEntry entry =
      (PortletEntry) Registry.getEntry(Registry.PORTLET, getName());
  ...
  /**
   * Implements the default close behavior: security permissions will be
   * checked.
   * 
   * @param rundata
   *          The RunData object for the current request
  ...
  /**
   * Implements the default customize behavior: security permissions will be
   * checked.
   * 
   * @param rundata
   *          The RunData object for the current request
  ...
  /**
   * Implements the default maximize behavior: security permissions will be
   * checked.
   * 
   * @param rundata
   *          The RunData object for the current request
  ...
  /**
   * Implements the default print friendly format behavior: security permissions
   * will be checked.
   * 
   * @param rundata
   *          The RunData object for the current request
   ...
   * 
   * @param minimize
   *          True if the portlet change to minimized
   * @param rundata
   *          The RunData object for the current request
   */
  @Override
  ...
  /**
   * Returns TRUE if the title bar in should be displayed. The title bar
   * includes the portlet title and action buttons. This
   * 
   * @param rundata
   *          The RunData object for the current request
   */
  ...
  public boolean isShowTitleBar(RunData rundata) {
    if (getPortletConfig() != null) {
      ...
      return Boolean
        .valueOf(getPortletConfig().getInitParameter("_showtitlebar", "true"))
        .booleanValue();
    }
   ...
   * @param attrDefValue
   *          The value if the attr doesn't exists
   * @param rundata
   *          The RunData object for the current request
  ...
  public void setAttribute(String attrName, String attrValue, RunData rundata) {
    try {
      ...
      logger.error("Exception while setting attribute "
        + attrName
        + " for portlet "
        + getName(), e);
    }
  ...
  //
  // DST: Shouldn't getID and setID be deprecated and added to
  // PortletInstance...
  //
  @Override
  public String getID() {
