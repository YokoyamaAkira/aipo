...
public class ServletInvokerPortlet extends AbstractPortlet {
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(ServletInvokerPortlet.class.getName());

  /**
   * Returns an ECS concrete element that includes the servlet/JSP.
   * 
   * The servlet/JSP will be invoked when the ECS tree is written to the servlet
   * output stream and add its output to the stream.
   */
  @Override
  public ConcreteElement getContent(RunData rundata) {
    ...
    try {
      ...
      String message =
        "ServletInvokerPortlet: Error invoking "
          + servletURL
          + ": "
          + e.getMessage();
