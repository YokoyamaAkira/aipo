...
public class WebClippingPortlet extends AbstractInstancePortlet {
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(WebClippingPortlet.class.getName());
  ...
  private final String BAD_PARAM = "<br>Error: startTag without stopTag<br>";

  // Error message for wrong startTagNumber parameter
  private final String BAD_NUMBER = "<br>Error: bad integer parameter<br>";
   ...
   */
  private ConcreteElement getWebClippedContent(RunData data,
    ...
    try {
      ...
      while (en.hasMoreElements()) {
        ...
        if (name.equals(START + String.valueOf(patternNumber))
          ...
          String start = patterns.get(START + String.valueOf(patternNumber));
          String simpleTag = patterns.get(TAG + String.valueOf(patternNumber));
          String stop = patterns.get(STOP + String.valueOf(patternNumber));
          String tagNum = patterns.get(TAGNUM + String.valueOf(patternNumber));
          // A group of params can have a specific url
          String url = patterns.get(URL + String.valueOf(patternNumber));
          ...
          if ((simpleTag != null) && (tagNum == null)) {
            clippedString =
              clippedString
                + Transformer.findElement(htmlReader, url, simpleTag);
            ...
            clippedString =
              clippedString
                + Transformer.clipElements(htmlReader, url, start, stop);
          } else if (tagNum != null) {
            clippedString =
              clippedString
      ...
      // FIXME: We should do a clearContent() for the media type, not ALL media
      // types
      this.clearContent();
    ...
    try {
      while (en.hasNext()) {
        ...
        if (name.equals("username")) {
          username = this.getPortletConfig().getInitParameter("username");
        } else if (name.equals("password")) {
          password = this.getPortletConfig().getInitParameter("password");
        } else {
          patterns.put(name, this.getPortletConfig().getInitParameter(name));
        }
