...
public class DaemonAdminPortlet extends AbstractPortlet {
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(DaemonAdminPortlet.class.getName());
    ...
    try {
      ...
      for (int i = 0; i < entries.length; ++i) {
        ...
        table.addElement(new TR()
          ...
          .addElement(
            new TD(this.getStatus(DaemonFactory.getStatus(entries[i]))))
          .addElement(
            new TD(this.getResult(DaemonFactory.getResult(entries[i]))))
        ...
        if (message != null) {
          ...
          table.addElement(new TR().addElement(new TD()
            .setColSpan(7)
            .addElement(message)));
  ...
  private ConcreteElement getFeedCount() {
    ...
    return new P().addElement(new B("Current number of feeds:  ")).addElement(
      Integer.toString(FeedDaemon.getCount()));
  ...
  private Form getForm(String daemon, RunData rundata) {
    ...
    form.addElement(new Input().setType("submit").setName(REFRESH).setValue(
      REFRESH));

    form.addElement(new Input().setType("hidden").setName(DAEMON).setValue(
      daemon));
