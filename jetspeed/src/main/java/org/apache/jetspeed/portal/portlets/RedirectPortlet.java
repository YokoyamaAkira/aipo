...
/**
 * RedirectPortlet can be used for menu options that redirect directly to a URL
...
public class RedirectPortlet extends AbstractInstancePortlet {
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(RedirectPortlet.class.getName());
  ...
  public ConcreteElement getContent(RunData rundata) {
    ...
    try {
      String script =
        "<script> setTimeout( \"location.href='" + url + "'\", 1) </script>";
  ...
  private String createDynamicUrl(JetspeedRunData rundata, String url) {
    String parameterNames =
      PortletConfigState.getParameter(this, rundata, "parameterNames", null);
    ...
    String[] attribNames =
      StringUtils.stringToArray(sessionAttributeNames, ",");
    ...
    int count =
      (names.length > attribNames.length) ? names.length : attribNames.length;
  ...
  /**
   * First we look in the request for it, if not found look in the session. If
   * it contains a '.' & an object is found in the one of the scopes, then we
   * try and call the getter method for the field defined after the '.'.
   * 
   * @param rundata
   * @param attributeName
