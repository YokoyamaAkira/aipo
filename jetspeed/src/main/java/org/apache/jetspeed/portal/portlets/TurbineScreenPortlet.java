 ...
 * </PRE>
 * 
 * In the above example, the parameter MYPARAM=MYVALUE will be set in the
 * rundata that calls the screen.
 * 
...
public class TurbineScreenPortlet extends AbstractPortlet {
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(TurbineScreenPortlet.class.getName());
    ...
    try {
      ...
      if (screen == null) {
        throw new PortletException("You need to specify a "
          + SCREEN
          + " parameter for this portlet");
      }

      /* Save the parameters if any */
      String param = null;
      String value = null;
      Map<String, String> dict = pc.getInitParameters();
  ...
  private RunData setParams(RunData data) {
    ...
    while (en.hasMoreElements()) {
      ...
      data.getParameters().add(param, paramSet.get(param));
    }
    return (data);
  }
}
