 ...
 * <li><b>using the "template" parameter</b> - the JspTemplateService will
 * search portlets and then screens folder to locate the appropriate template.
 * The template must be specifed in the "template" portlet parameter.
 * <li><b>using relative url</b> - the .jsp template will be served directly
 * bypassing the JspTemplateService. The template must be specifed in the
 * portlet url property. Example: /html/welcome.jsp.
 * <P>
 * 
 ...
 * @version $Id: $
 */
public class JSPViewProcessor implements ViewProcessor {

  /**
   * Static initialization of the logger for this class
   */
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(JSPViewProcessor.class.getName());
  ...
  public Object processView(GenericMVCContext context) {
    ...
    try {
      ...
      // Add rundata out of convenience (JspService.RUNDATA differs from
      // GenericMVCPortlet.RUNDATA)
      ...
      if (pe.getURL() == null || pe.getURL().trim().length() == 0) {
        ...
        /*
         * if (locatedTemplate == null) { locatedTemplate =
         * TemplateLocator.locateScreenTemplate(data, template); if
         * (locatedTemplate != null) { locatedTemplate = "/screens" +
         * locatedTemplate; }
        ...
        JspService service =
          (JspService) ServiceUtil.getServiceByName(JspService.SERVICE_NAME);
        ...
        if (logger.isDebugEnabled()) {
          logger.debug("JSPViewProcessor - serving jsp directly using: "
            + template);
        }
   ...
   * Invoked by the GenericMVCPortlet after action handling to process the
   * template type in question.
   * 
   */
  @Override
