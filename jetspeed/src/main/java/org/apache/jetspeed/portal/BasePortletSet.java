...
/**
 * The PortletSet is basically a wrapper around an array of portlets. It
 * provides runtime context for a set of portlets. A portlet can get its current
 * set by calling via its PortletConfig
 * 
...
public class BasePortletSet implements PortletSet, Portlet, PortletState {
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(BasePortletSet.class.getName());
  ...
  public ConcreteElement getContent(RunData rundata) {
    ...
    if (!JetspeedSecurity.checkPermission(
      ...
      if (logger.isDebugEnabled()) {
        logger.debug("Unauthorized access by user \""
      ...
      return new StringElement(Localization.getString(
        rundata,
        "SECURITY_NO_ACCESS"));
    } else {
      if (logger.isDebugEnabled()) {
  ...
  /**
   * Get a title for this instance of the portlet. This method is called from
   * the context variable portlet_instance and from PortletInstance
   * 
   * If you wish to append to the title, then you code should look like
   * getTitle( String instanceTitle) { return super.getTitle( instanceTitle) +
   * " - Appened title text"; }
   * 
   * @param instanceTitle
   *          Title from PSML
  ...
  public final void sortPortletSet() {
    ...
    synchronized (portlets) {
      for (int i = 0; i < portlets.size(); i++) {
        ...
        Portlet p = portlets.elementAt(i);
  ...
  /**
   * Method retruns true if at least one of the portlets of the portletset fits
   * the requested MimeTyp. Otherwise it retruns false.
   */
  @Override
  public boolean supportsType(MimeType mimeType) {
    Enumeration<Portlet> portlets = this.getPortlets();
    while (portlets.hasMoreElements()) {
      Portlet p = portlets.nextElement();
  ...
  /**
   * Implements the default print friendly format behavior: not available for
   * the portlet set
   * 
   * @param rundata
   *          the RunData object for the current request
  ...
  public boolean allowCustomize(RunData rundata) {
    ...
    return JetspeedSecurity.checkPermission(
