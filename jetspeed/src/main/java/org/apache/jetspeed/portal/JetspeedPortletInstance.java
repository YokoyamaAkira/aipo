...
/**
 * Default Implementation of Jetspeed Portlet Instance. It implements the
 * methods for getting, setting and removing attributes from portlet instance
 * persistence storage.
 * 
 * In a Jetspeed 1.4x PSML profile, the default XML format for an instance and
 * attribute is:
 * 
 * <entry> <parameter name="someName" value="someValue"/>
 * 
 * Since this modifies PSMLDocuments, it will make a clone once modification
 * starts.
 * 
 * @author <a href="mailto:sweaver@rippe.com">Scott Weaver</a>
 * @author <a href="mailto:ggolden@apache.org">Glenn R. Golden</a>
 * @version $Id: JetspeedPortletInstance.java,v 1.9 2004/02/23 04:05:35 jford
 *          Exp $
 */
public class JetspeedPortletInstance implements PortletInstance {
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(JetspeedPortletInstance.class.getName());
   ...
   * Rundata objects are re-used, and are only good within one request. This
   * means that a PortletInstance CANNOTA BE STORED BEYOND A SINGLE REQUEST!
   */
  private JetspeedRunData m_rundata = null;

  /**
   * keeping an instance of the actual Portlet around helps us with getting
   * information that is not directly available from the PSML profile
   */
   ...
   * 
   * @param Portlet
   *          The parent portlet of this instance.
   * @param RunData
   *          Jetspeed Run Data
   */
   ...
   *          The parent portlet of this instance.
   * @param Profile
   *          The profile containing the instance.
   */
  ...
  /**
   * Protect the constructor so it can only be instantitated by by the
  ...
  public Profile getProfile() {
    ...
    if (m_clonedProfile != null) {
      return m_clonedProfile;
    }
  ...
  /**
   * Setup for making a change to the Entry We must not change the PSMLDocument
   * that we started with, instead we make a clone and start using it.
   */
