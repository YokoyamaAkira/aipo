 ...
 * Please note that currently only inquiry tasks are supported. I hope to add
 * other task types soon.
 *
...
public class UDDIAccessBean extends Object implements Serializable {
  ...
  private static final JetspeedLogger logger =
    JetspeedLogFactoryService.getLogger(UDDIAccessBean.class.getName());
    ...
    try {
      ...
      if (logger.isDebugEnabled()) {
        ...
        while (iterator.hasNext() == true) {
          org.uddi4j.response.BusinessInfo business =
            (org.uddi4j.response.BusinessInfo) iterator.next();
  ...
  public java.util.List queryService(String businessKey, String description) {
    ...
    try {
      proxy.setInquiryURL(queryURL);
      ServiceList results =
        proxy.find_service(businessKey, description, null, 0);
      serviceList = results.getServiceInfos().getServiceInfoVector();
      if (logger.isDebugEnabled()) {
        java.util.ListIterator iterator = serviceList.listIterator();
        while (iterator.hasNext() == true) {
          org.uddi4j.response.ServiceInfo service =
            (org.uddi4j.response.ServiceInfo) iterator.next();
  ...
  public String getServiceKey(String businessKey, String serviceName) {
    ...
    try {
      proxy.setInquiryURL(queryURL);
      ServiceList results =
        proxy.find_service(businessKey, serviceName, null, 0);
      serviceList = results.getServiceInfos().getServiceInfoVector();
      // create iterator to search for match
      java.util.ListIterator iterator = serviceList.listIterator();
      while (iterator.hasNext() == true) {
        ...
        if (service.getNameString().equals(serviceName)) {
          ...
          if (logger.isDebugEnabled()) {
            logger
              .debug(
                service.getNameString() + " has associated key " + serviceKey);
          }
        }
      }
    } catch (java.net.MalformedURLException exception) {
