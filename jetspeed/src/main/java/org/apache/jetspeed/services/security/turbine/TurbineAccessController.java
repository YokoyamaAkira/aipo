 ...
 * @version $Id: TurbineAccessController.java,v 1.8 2004/02/23 03:54:49 jford
 *          Exp $
 */
public class TurbineAccessController extends TurbineBaseService implements
    PortalAccessController {
  /**
   * Static initialization of the logger for this class
   */
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(TurbineAccessController.class.getName());
  ...
  public boolean checkPermission(JetspeedUser user, PortalResource resource,
    ...
    switch (resource.getResourceType()) {
      ...
      case PortalResource.TYPE_ENTRY_PARAMETER:
        return checkPermission(user, (RegistryEntry) resource
          .getEntryParameter(), action);
    }
    return false;
  }
   ...
   */
  /**
   * given the rundata, checks if the currently logged on user has access for
   * the given action
   * 
   * @param rundata
   *          the request rundata.
  ...
  private boolean checkDefaultPermission(JetspeedUser user, String action) {
    ...
    try {
      ...
      logger.error(
        "Error checking permissions for " + user + " on " + action,
        e);
    }
