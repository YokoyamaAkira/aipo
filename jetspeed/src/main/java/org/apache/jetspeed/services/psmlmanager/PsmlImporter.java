...
public class PsmlImporter {
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(PsmlImporter.class.getName());
  ...
  public static void main(String args[]) {
    ...
    if (exporterService.getClass().getName().equals(
      importerService.getClass().getName())) {
      String msg =
        "PSML Importer Error: Importer Class cannot equal Exporter Class.";
      logger.error(msg);
      System.out.println(msg);
      System.exit(0);
    }
  ...
  /*
   * Check to see if import has already completed. Only considers a "onetime"
   * import, checking for the "admin" user.
   * 
  ...
  public boolean alreadyImported() {
    try {
      ...
      if (iterator.hasNext()) {
        String msg =
          "PSMLImporter: Detected database is populated. No need to import.";
        System.out.println(msg);
        logger.info(msg);
      ...
      String msg =
        "Failed to run import: Database Access Error detecting database on import: ";
      logger.error(msg, e);
