...
/**
 * <p>
 * This service extends the TurbineTemplateService to modify its behaviour: Not
 * only layout and screen packages, but also the screen templates are searched
 * in the template neames filepath, so that a fallback strategy is provided,
 * that can be used for multi-language, multi-device and browser-specific
 * support support.
 * </p>
 * <p>
 * E.g: a template name "/html/en/US/IE/mytemplate" would search for following
 ...
 * @version $Id: JetspeedTemplateService.java,v 1.11 2004/02/23 03:38:54 jford
 *          Exp $
 */
...
// removed dst: 2001/06/03, TDK 2.2 integration
{
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(JetspeedTemplateService.class.getName());
  ...
  /**
   * The absolute paths where the appropriate template engine will be searching
   * for templates.
   */
  ...
  public void init(ServletConfig config) throws InitializationException {
    try {
      ...
      throw new InitializationException(
        "TurbineTemplateService failed to initialize",
        e);
    }
  }
  ...
  private void initTemplate(ServletConfig config) throws Exception {
    ...
    if (useCache) {
      int layoutSize =
        Integer.parseInt(props.getProperty("layout.cache.size", "5"));
      int navigationSize =
        Integer.parseInt(props.getProperty("navigation.cache.size", "10"));
      int screenSize =
        Integer.parseInt(props.getProperty("screen.cache.size", "5"));
      int templateSize =
        Integer.parseInt(props.getProperty("screen.cache.size", "50"));
      layoutCache =
        new Hashtable<String, Object>((int) (1.25 * layoutSize) + 1);
      navCache =
        new Hashtable<String, Object>((int) (1.25 * navigationSize) + 1);
      screenCache =
        new Hashtable<String, Object>((int) (1.25 * screenSize) + 1);
      templateCache =
        new Hashtable<String, Object>((int) (1.25 * templateSize) + 1);
    }
    ...
    templatePaths = ServletUtils.expandRelative(config, templatePaths);
    ...
    // the default modules
    defaultNavigation =
      props.getProperty("default.navigation", "TemplateNavigation");
    defaultScreen = props.getProperty("default.screen", "TemplateScreen");

    // the default layout template
    defaultLayoutTemplate =
      props.getProperty("default.layout.template", "/default." + extension);
   ...
   * 
   * @param key
   *          The String key for the object.
   * @param value
   *          The Object.
   * @param h
   *          The Hashtable.
   */
  private void addToCache(String key, Object value, Hashtable<String, Object> h) {
   ...
   * 
   * @return A String which is the value of the TemplateService default.screen
   *         property.
   */
  ...
  /**
   * This method takes the template parameter and parses it, so that relevant
   * Screen/Layout-template information can be extracted.
   * 
   * @param template
   *          A String with the template name.
   * @return A String[] where the first element is the Screen name and the
   *         second element is the layout template.
   */
  protected String[] parseScreenTemplate(String template) throws Exception {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug("JetspeedTemplateService.parseScreen: template = "
        + template);
    }
    ...
    for (int i = 0; i < templateRoot.length; i++) {
      if (logger.isDebugEnabled()) {
        logger.debug("JetspeedTemplateService.parseScreen: templateRoot "
          + i
          + " "
          + templateRoot[i]);
      }
      ...
      for (int k = tokens.size(); k >= 0; k--) {
        ...
        for (int j = 0; j < k; j++) {
          path.append("/").append(tokens.get(j));
        }
        StringBuffer distinctPath =
          new StringBuffer(path.toString()).append("/").append(fileName);
        templatePath = distinctPath.toString();
        if (logger.isDebugEnabled()) {
          logger.debug("JetspeedTemplateService.parseScreen: Path: "
            + templatePath);
        }
        ...
        if (new File(templateRoot[i] + "/screens" + templatePath).exists()) {
          ...
          if (logger.isDebugEnabled()) {
            logger
              .debug("JetspeedTemplateService.parseScreen: template found: "
                + template);
          }
          break;
        }
      ...
      if (templatePath != null) {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug("JetspeedTemplateService.parseScreen: pathRoot: "
            + pathRoot);
        }
        break;
      }
    ...
    /*
     * String[] paths = new String[tokens.size() + 2]; String[] pkgs = new
     * String[tokens.size() + 2]; int arrayIndex = 0; for (int i=tokens.size();
     * i>=0; i--) { StringBuffer path = new StringBuffer(); StringBuffer pkg =
     * new StringBuffer(); for (int j=0; j<i; j++) {
     * path.append("/").append((String)tokens.get(j));
     * pkg.append((String)tokens.get(j)).append('.'); } if ( i == tokens.size()
     ...
     * arrayIndex++; } paths[arrayIndex] =
     * path.append(defaultLayoutTemplate).toString(); pkgs[arrayIndex] =
     * pkg.append("Default").toString(); arrayIndex++; }
     */
    ...
    for (int i = tokens.size(); i >= 0; i--) {
      ...
      for (int j = 0; j < i; j++) {
        path.append("/").append(tokens.get(j));
        pkg.append(tokens.get(j)).append('.');
      }
    ...
    if (logger.isDebugEnabled()) {
      for (int i = 0; i < paths.length; i++) {
        logger.debug("JetspeedTemplateService.parseScreen: paths["
          + i
          + "] = "
          + paths[i]);
      }
  ...
  /**
   * Parse the template name out to a package path to locate the Navigation
   * module. This is different than the Screen/Layout parser in that it only
   * looks for packages. Note: If caching is enabled, this is only performed
   * once for each unique template.
   * 
   * @param String
   *          The template name (i.e folder/headernav.wm).
   * @return A String with the name of the Navigation module to use for the
   *         template.
   */
   ...
   * 
   * @param possiblePaths
   *          A String[] with possible paths to search.
  ...
  private String getLayoutTemplateName(String pathRoot, String[] possiblePaths) {
    if (logger.isDebugEnabled()) {
      logger.debug("JetspeedTemplatePage.getLayoutTemplateName: pathRoot "
        + pathRoot);

      for (int i = 0; i < possiblePaths.length; i++) {
        logger
          .debug("JetspeedTemplatePage.getLayoutTemplateName: possiblePaths["
            + i
            + "]="
            + possiblePaths[i]);
      }
    ...
    for (int i = 0; i < possiblePaths.length; i++) {
      if (new File(pathRoot, "layouts" + possiblePaths[i]).exists()) {
        if (logger.isDebugEnabled()) {
          logger.debug("JetspeedTemplatePage.getLayoutTemplateName: "
            + pathRoot
            + "/layouts"
            + possiblePaths[i]
            + " found.");
  ...
  /**
   * Seaches for the Navigation class that may match the name of the Navigation
   * template.
   * 
   * @param possibleNavigations
   *          A String[] with possible navigation packages.
