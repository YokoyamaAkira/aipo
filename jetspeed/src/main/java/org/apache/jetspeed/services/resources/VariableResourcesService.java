 ...
 * sequence of tokens: ${<varname>}. Varname is always folded to lowercase.
 * </p>
 * <P>
...
public class VariableResourcesService extends TurbineResourceService {
  ...
  private void initVariables(ServletConfig config) {
    ...
    /*
     * path = ctxt.getContext("/").getRealPath("/"); if (path != null ) {
  ...
  public Vector<String> getVector(String name) {
    ...
    if (std == null) {
      ...
      if (std != null) {
        ...
        while (en.hasMoreElements()) {
          newstd.addElement(substituteString(en.nextElement()));
        }
        std = newstd;
