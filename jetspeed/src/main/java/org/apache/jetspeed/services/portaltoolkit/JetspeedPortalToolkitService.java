 ...
 * 
 * @version $Id: JetspeedPortalToolkitService.java,v 1.33 2004/03/29 21:02:29
 *          taylor Exp $
 */
public class JetspeedPortalToolkitService extends TurbineBaseService implements
    PortalToolkitService {
  /**
   * Static initialization of the logger for this class
   */
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(JetspeedPortalToolkitService.class.getName());
  ...
  public void init(ServletConfig conf) throws InitializationException {
    ...
    this.defaultUserSecurityRef =
      serviceConf.getString("default.user.security.ref");
    this.defaultAnonSecurityRef =
      serviceConf.getString("default.anon.security.ref");
    this.defaultRoleSecurityRef =
      serviceConf.getString("default.role.security.ref");
    this.defaultGroupSecurityRef =
      serviceConf.getString("default.group.security.ref");
    setInit(true);

  }

  /**
   * Instanciates a PortletControl based on a Registry entry, if available or
   * directly from a classname.
   * 
   * @param name
  ...
  public PortletControl getControl(String name) {
    ...
    try {
      ...
      logger.error(
        "Unable to instanciate control " + name + ", using default",
        e);
    }
  ...
  /**
   * Instanciates a PortletController based on a Registry entry, if available or
   * directly from a classname.
   * 
   * @param name
  ...
  public PortletController getController(String name) {
    ...
    try {
      ...
      logger.error("Unable to instanciate controller "
        + name
        + ", using default");
    }
  ...
  /**
   * Creates a PortletSet from a PSML portlets description, updating the
   * portletset name based on its position within the tree
   * 
   * @param portlets
   *          the PSML portlet set description
   * @param count
   *          the portletset number within the complete tree
   * @return a new instance of PortletSet
   */
        ...
        if (entry != null) {
          ...
          if (p != null) {
            Map<String, String> constraints =
              getParameters(psmlEntry.getLayout());
  ...
  /**
   * Associates a PortletControl with an existing Portlet and returns the
   * Control
   * 
   * @param pc
   *          the existing PortletControl
   * @param portlet
   ...
   * 
   * @param entry
   *          the Portlets containing the config
   ...
   * @return a Map containing the names/values, an empty map is returned if
   ...
   * <code>Portlets</code> collection.</li>
   ...
   * 
   * @param Portlets
   *          portlets Portlets collection whose skin needs to be located.
   */
  ...
  public SecurityReference getDefaultSecurityRef(String type) {
    ...
    if (defaultRef != null) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug("JetspeedPortalToolkit: default security for type: "
          + type
          + " is "
          + defaultRef);
      }
