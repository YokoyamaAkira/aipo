...
public class WebPageCache {
  ...
  // The Cache
  private static HashMap<String, CachedResource> cache =
    new HashMap<String, CachedResource>();

  // the log file singleton instance
  static Logger log = Logger.getLogger(WebPageCache.class);

  /**
   * Given a cacheable web resource, writes the content of that resource to the
   * servlet output stream. The first time that the resource is requested, the
   * content is fetched from the proxied host. From then on, the resource
   * content is served up from the cache.
   * 
   * @param resource
   ...
   * @param base
   *          the proxied host's base URL.
   * @param host
   *          the Proxy Server base URL.
   * @param data
   *          the RunData for the request.
   * @return boolean true if the resource was read from the cache, false if read
   *         from the server.
   */
  public static boolean getResourceFromCache(String resource, long sid,
      String base, String host, ProxyRunData data) {
    try {
      CachedResource cr = cache.get(resource);
  ...
  /**
   * Determines if a resource is cacheable, dependent on the extension: defined
   * in CACHEABLE_RESOURCES (gif, jpeg, jpg, png, js, css)
   * 
   * @param resource
   *          the resource that is being proxied.
  ...
  /**
   * Retrieves the content from the proxied host for the requested. Per
   * cacheable resource, this is only called once. All further requests will
   * return the cached content. The content is immediately written to the
   ...
   */
  public static byte[] getContentAndWrite(URLConnection con, ProxyRunData data)
      throws IOException {
    int CAPACITY = 4096;
  ...
  /**
   * Retrieves the script content from the proxied host for the requested. Per
   * cacheable resource, this is only called once. All further requests will
   * return the cached content. The content is first rewritten, rewriting all
   * links found in the script back to the Proxy server. Then, the content is
   * immediately written to the servlet's response output stream.
   * 
   * @param con
   *          the HTTP connection to the proxied host.
   * @param response
   ...
   * @return byte[] the resource content, which will be stored in the cache.
   */
  public static byte[] rewriteScript(URLConnection con, long sid, String host,
      ProxyRunData data, String resource, String base) throws IOException {
    int CAPACITY = 4096;
    ...
    if (sid == -1) { // FIXME: I seem to have lost this code....
                     // return HTMLRewriter.rewriteScript(script, resource,
                     // host, base);
    }
