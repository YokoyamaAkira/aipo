...
/**
 * <p>
 * The <code>NoRoleManagement</code> class is a Jetspeed security provider,
 * implementing the <code>RoleManagement</code> interface. It provides no role
 * management - only the "user" role exists for any user, no roles are listed or
 * saved, any role requested is supplied with a temp. Role object.
 * 
 ...
 */
public class NoRoleManagement extends TurbineBaseService implements
    RoleManagement {
  /**
  ...
  public boolean hasRole(String username, String rolename, String groupname)
    ...
    // give everyone the "user" role
    if (rolename.equals(JetspeedSecurity.JETSPEED_ROLE_USER)
      && groupname.equals(JetspeedSecurity.JETSPEED_GROUP)) {
      return true;
    }
