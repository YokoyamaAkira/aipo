...
/**
 * <p>
 * The <code>TurbineAuthentication</code> class is a default Jetspeed security
 * provider, implementing the <code>PortalAuthentication</code> interface. It
 * provides authentication services using a User database table mimicking the
 * legacy Turbine-2 user table.
 *
 * This service does not use any of the Turbine security or user management
 * classes.
 *
...
public class TurbineAuthentication extends TurbineBaseService implements
    ...
    if (this.expirationPeriod > 0) {
      ...
      if (passwordLastChangedDate != null) {
        ...
        if (logger.isDebugEnabled()) {
          logger
            .debug(
              "TurbineAuthentication: password last changed = "
   ...
   * <code>Principal</code> and <code>Credential</code> information if relevant
  ...
  public synchronized void init(ServletConfig conf)
    ...
    expirationPeriod =
      serviceConf
        .getInt(TurbineAuthentication.CONFIG_PASSWORD_EXPIRATION_PERIOD, 0);
