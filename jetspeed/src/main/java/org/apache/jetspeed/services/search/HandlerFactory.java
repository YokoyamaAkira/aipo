...
public class HandlerFactory {
  private static final Map<String, ObjectHandler> handlerCache = Collections
    .synchronizedMap(new HashMap<String, ObjectHandler>());
  ...
  public static ObjectHandler getHandler(String className) throws Exception {
    ...
    if (handlerCache.containsKey(className)) {
      handler = handlerCache.get(className);
    } else {
    ...
    // System.out.println("HandlerFactory: returning handler " + handler +
    // " for " + obj);
