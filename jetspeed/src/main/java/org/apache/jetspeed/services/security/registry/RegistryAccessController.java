 ...
 * @version $Id: RegistryAccessController.java,v 1.10 2004/02/23 03:54:03 jford
 *          Exp $
 */
public class RegistryAccessController extends TurbineBaseService implements
    PortalAccessController {
  /**
   * Static initialization of the logger for this class
   */
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(RegistryAccessController.class.getName());

  /**
   * Given a <code>JetspeedUser</code>, authorize that user to perform the
   * secured action on the given <code>Portlet</code> resource. If the user does
   * not have sufficient privilege to perform the action on the resource, the
   * check returns false, otherwise when sufficient privilege is present,
    ...
    if (registryEntry == null) {
      ...
      if (ps != null) {
        ...
        if (pc != null) {
          ...
          registryEntry =
            Registry.getEntry(Registry.PORTLET_CONTROLLER, portletName);
        }
      }
    }
    if (registryEntry == null) {
      return true; // Since their is no entry, their no security to test. Per
                   // spec. all is allowed
    }
  ...
  /**
   * Given a <code>JetspeedUser</code>, authorize that user to perform the
   * secured action on the given Portlet Instance (<code>Entry</code>) resource.
   * If the user does not have sufficient privilege to perform the action on the
   * resource, the check returns false, otherwise when sufficient privilege is
   * present, checkPermission returns true.
   * 
   * @param user
   *          the user to be checked.
   * @param entry
   *          the portlet instance resource.
   * @param action
   *          the secured action to be performed on the resource by the user.
   * @return boolean true if the user has sufficient privilege.
   */
  ...
  public boolean checkPermission(JetspeedUser user, Entry entry, String action,
    ...
    if (securityRef == null) {
      return checkPermission(user, Registry.getEntry(Registry.PORTLET, entry
        .getParent()), action, owner);
    }
    return checkPermission(user, securityRef, action, owner);
  }
  ...
  /**
   * Given a <code>JetspeedUser</code>, authorize that user to perform the
   * secured action on the given resource. If the user does not have sufficient
  ...
  public boolean checkPermission(JetspeedUser user, PortalResource resource,
    ...
    switch (resource.getResourceType()) {
      case PortalResource.TYPE_ENTRY:
        return checkPermission(user, resource.getEntry(), action, resource
          .getOwner());
      case PortalResource.TYPE_PORTLET:
        return checkPermission(user, resource.getPortlet(), action, resource
          .getOwner());
      case PortalResource.TYPE_REGISTRY:
        return checkPermission(
          user,
          resource.getRegistryEntry(),
          action,
          resource.getOwner());
      case PortalResource.TYPE_REGISTRY_PARAMETER:
        return checkPermission(
          user,
          resource.getRegistryParameter(),
          action,
          resource.getOwner());
    }
    ...
    logger.error("In "
      ...
      + ".checkPermission(user, resource, action) - Unkown resource = "
      + resource.getResourceType());
    return false;
  }

  /**
   * Checks if the user has access to a given registry entry for the given
   * action
   * 
   * @param user
   *          the requesting user.
   * @param regEntry
   *          the registry entry from the registry.
   * @param owner
   *          of the entry, i.e. the username
   * @param action
   *          the jetspeed-action (view, edit, customize, delete...) for which
  ...
  /**
   * Checks if the user has access for the given action using a security
   * reference
   * 
   * @param user
   *          the requesting user.
   * @param securityRef
   *          the security reference to check
   * @param action
   *          the jetspeed-action (view, edit, customize, delete...) for which
   *          permission is being checked.
   */
  private boolean checkPermission(JetspeedUser user,
      SecurityReference securityRef, String action, String owner) {
    SecurityEntry securityEntry =
      (SecurityEntry) Registry.getEntry(Registry.SECURITY, securityRef
        .getParent());
    if (securityEntry == null) {
      logger.warn("Security id "
        + securityRef.getParent()
        + " does not exist.  This was requested by the user "
        + user.getUserName());
      return false;
    }
    ...
    try {
      for (Iterator<?> roles =
        JetspeedRoleManagement.getRoles(user.getUserName()); roles.hasNext();) {
      ...
      //
      // for( Iterator groups =
      // JetspeedGroupManagement.getGroups(user.getUserName());
      // groups.hasNext();)
      // {
