...
public class TurbineRoleManagement extends TurbineBaseService implements
    RoleManagement {
   ...
   * @return Iterator over all roles associated to the user principal (iterator
   *         of GroupRole objects keyed on group+role).
  ...
  public Iterator<?> getRoles(String username) throws JetspeedSecurityException {
    ...
    try {
      ...
      if (rels.size() > 0) {
        roles = new HashMap<String, Object>(rels.size());
      } else {
