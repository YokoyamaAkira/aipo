...
public class CachedAcl {
  protected HashMap<String, GroupRole> groupRoles =
    new HashMap<String, GroupRole>();
  ...
  public Role getRole(String roleName, String groupName) {
    GroupRole gr = groupRoles.get(getGroupRoleKey(groupName, roleName));
  ...
  public void setRoles(Iterator<?> grouproles) {
    while (grouproles.hasNext()) {
      ...
      String key =
        getGroupRoleKey(grouprole.getGroup().getName(), grouprole
          .getRole()
          .getName());
