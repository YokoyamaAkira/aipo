 ...
 * BadURLManager. There are also some util methods for downloading URLs that
 * don't use the Disk Cache.
 * </p>
 *
 *
 *
...
public class URLFetcher {
  ...
  private static final JetspeedLogger logger =
    JetspeedLogFactoryService.getLogger(URLFetcher.class.getName());

  /**
   * URLs that Jetspeed is currently trying to fetch in real time.
   */
  private static Hashtable<String, Vector<Thread>> realtime_urls =
    new Hashtable<String, Vector<Thread>>();
  ...
  /**
   * Try and fetch a URL as and get the content as a String and possibly add the
   * URL to the BadURLManager if anything goes wrong.
   *
   * @param url
   *          The URL to fetch
   * @param force
   *          if set to true then do not use force this entry to be in the
   *          cache... IE do not use CACHE_REQUIRE_CACHED
   */
  public static final Reader fetch(String url, boolean force)
      throws IOException {
    ...
    try {
      ...
      if (proxyHost != null) {
        ...
        content =
          new URL(protocol, proxyHost, URLManager.getProxyPort(protocol), url);
      } else {
        content = new URL(url);
      }
  ...
  /**
   * Try and fetch a URL if the copy in the cache has expired and add the URL to
   * the BadURLManager if anything goes wrong.
   *
   * @param url
   *          The URL to fetch
   * @param force
  ...
  public static final boolean refresh(String url) throws IOException {
    ...
    if (DiskCacheUtils.isCached(url)) {
      try {
        ...
        if (respCode != 304
          /* NOT MODIFIED */ && (clength == -1 || clength > 0)
          && (last == 0 || last > dce.getLastModified())) {

          logger
            .info(
              "URLFetcher: Found updated URL: "
                + url
                + " Modified "
                + last
                + " Expires: "
          ...
          logger
            .info(
              "DiskCacheDaemon: URL still valid: "
                + url
                + " Modified "
                + last
                + " Expires: "
        ...
        URLManager.register(url, URLManagerService.STATUS_BAD, e.toString());
      } finally {
        removeRealtimeURL(url);
      }
      ...
      logger
        .info("URLFetcher: Cache miss during validation! Forcing url: " + url);
      removeRealtimeURL(url);
  ...
  /**
   *
   * Return a Reader for a given HTTP connection. If the connection first line
   * contains a XML declaration with encoding, honor this encoding. If not, use
   * the encoding from the HTTP connection, taking ISO-8859-1 as default.
   *
   */
  static final Reader getReader(URLConnection conn) throws IOException,
    ...
    is.mark(20480);
    BufferedReader asciiReader =
      new BufferedReader(new InputStreamReader(is, "ASCII"));
  ...
  static final void addRealtimeURL(String url) {
    synchronized (realtime_urls) {
      Vector<Thread> threads = realtime_urls.get(url);
      if (threads != null) {
  ...
  static final void removeRealtimeURL(String url) {
    synchronized (realtime_urls) {
      Vector<?> threads = realtime_urls.get(url);
      if (threads != null) {
