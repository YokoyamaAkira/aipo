...
/**
 * Stores a map of sessions with other sites for the current portal session. The
 * map is keyed by the portal session's SessionID (from the servlet api). Binds
 * to the servlet's session so that it can be cleaned up on session end. This
 * session keeps a map of all live sessions per site local session.
 * 
 * @author <a href="mailto:taylor@apache.org">David Sean Taylor</a>
 * @version $Id: SessionMap.java,v 1.3 2004/02/23 03:46:26 jford Exp $
 */

public class SessionMap extends HashMap<Object, Object> implements
   ...
   * 
   * @param sessionId
   *          the sessionID on this host
   * @param sessionUser
   *          the user associated with the new session.
   * 
   */
  ...
  /**
   * This method is a session binding event callback. It is called when the
   * session is bound
   * 
   */
  @Override
  ...
  /**
   * This method is a session unbinding event callback. It is called when the
   * session is unbound. We need to clean up all the site sessions associated
   * with this Session. The session is marked as 'not active' and is no longer
   * useable. It session object is then removed from the agent's cache
   * 
   */
  @Override
  public void valueUnbound(HttpSessionBindingEvent event) {
    log.info("~~~ SessionMap UNBOUND as "
      + event.getName()
      + " from "
