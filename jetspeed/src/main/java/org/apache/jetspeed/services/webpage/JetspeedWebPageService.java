 ...
 * 
 * <p>
 * This is the default implementation of the <code>WebPageService</code>
 * interface.
 * </p>
 * 
 * <p>
 * It is a service that provides Web Page facade and delegation services for
 * clients to transparently access resources existing on web pages from requests
 * originating from the portal server.
 * </p>
 * 
 * <p>
 * Since the WebPage service is giving the appearance of a single session to the
 * client, the service needs to manage the synchronization of sessions,
 * including single-sign-on, and security authorization permissions between the
 * the portal server and one or more sites.
 * </p>
 * 
...
public class JetspeedWebPageService implements WebPageService {
  ...
  private final HashMap<Object, Object> sites = new HashMap<Object, Object>();
  ...
  private final HashMap<String, SessionMap> sessions =
    new HashMap<String, SessionMap>();
  ...
  private String lastError =
    "Jetspeed WebPage Service has not been initialized.";
   ...
   */
  @Override
  public void get(HttpServlet servlet, HttpServletRequest request,
   ...
   */
  @Override
  public void post(HttpServlet servlet, HttpServletRequest request,
      HttpServletResponse response) throws ServletException, IOException {
  ...
  private void dispatch(ProxyRunData data) throws ServletException, IOException {
    ...
    if (null == sid) {
      if (null == url) {
        throw new ServletException(
          "Bad Request. No proxy-parameters passed in request.");
      }
      proxyByURL(url, data);
      return;
    }
    ...
    if (null == site) {
      ...
      throw new ServletException(
        "The Requested Site ID is currently not configured on this system: "
          + sid);
    }
    ...
    SessionMap smap = sessions.get(sessionID);
    SiteSession jss = null;
    if (null == smap) {
      // get the user from the session
      /*
       * User user = (User)session.getAttribute(config.getUserSessionKey());
       * String username; if (user != null) { username = user.getUserName(); }
       */
    ...
    if (loggedOn) {
      ...
      if (WebPageCache.isCacheableResource(resource)) {
        if (WebPageCache.getResourceFromCache(resource, site.getID(), site
          .getURL(), this.host, data)) {
          smap.incCacheCount();
          jss.incCacheCount();
        } else {
          smap.incHitCount();
  ...
  /**
   * Creates the full path the requested resource on the site from a relative
   * path in request.
   * 
   * @param url
  ...
  /**
   * Given a URL, begin a Jetspeed session with that host It is here for future
   * use.
   * 
   * @param url
   *          the URL of the host to proxy.
   * @param data
   *          the runData
   * @return a new session
   */
  private SiteSession proxyByURL(String url, ProxyRunData data)
  ...
  /**
   * Maps a full URL path to a resource to a base path given:
   * http://localhost:8080/jetspeed/search/index.html returns:
  ...
  public String getTargetBase(String url) throws ServletException {
    try {
      ...
      if ((port > 0)
        && ((protocol.equals("http") && port != 80) || (protocol
      ...
      if (null != path) {
        ...
        if (dot > slash && slash != -1) { // its a file
   ...
   */
  @Override
  public boolean init(ServletConfig config) throws ServletException,
      IOException {

    String paramFile = config.getInitParameter(INIT_PROPERTIES_PARAM);
    if (null == paramFile) {
      lastError =
        "Jetspeed HTTP Proxy Init Property Not Found:" + INIT_PROPERTIES_PARAM;
      log.error(lastError);
      return false;
    }
