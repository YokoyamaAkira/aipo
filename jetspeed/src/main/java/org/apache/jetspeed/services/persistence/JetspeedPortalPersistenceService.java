...
/**
 * Implementation of the Portal Persistence Service for storing and retrieving
 * portlet instances.
 * 
 * @author <a href="mailto:sweaver@rippe.com">Scott Weaver</a>
 * @version $Id: JetspeedPortalPersistenceService.java,v 1.5 2004/02/23 03:33:52
 *          jford Exp $
 */
public class JetspeedPortalPersistenceService extends TurbineBaseService
    implements PortalPersistenceService {
  /**
   * Static initialization of the logger for this class
   */
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(JetspeedPortalPersistenceService.class.getName());
  ...
  public void init(RunData data) throws InitializationException {
    if (!(data instanceof JetspeedRunData)) {
      logger
        .error("[PortalPersistenceService] The current RunData implenetation does not implement the JetspeedRunData interface.");
      setInit(false);
      throw new InitializationException(
        "The current RunData implenetation does not implement the JetspeedRunData interface.");
    }
  ...
  public PortletInstance getInstance(Portlet portlet, RunData data) {
    ...
    PortletInstance instance =
      (PortletInstance) data.getRequest().getAttribute(attrKey);
    if (instance != null) {
      return instance;
    } else {
  ...
  public void store(PortletInstance pPortlet) throws PortalPersistenceException {
    ...
    if (!PsmlManager.store(profile)) {
      throw new PortalPersistenceException(
        "Unable to write portlet information.");
    }
  }
   ...
   */
  @Override
  public List<JetspeedPortletInstance> getInstances(Profile profile)
      throws PortletException {
    PSMLDocument doc = profile.getDocument();
    Portlets portlets = doc.getPortlets();
    ArrayList<JetspeedPortletInstance> pList =
      new ArrayList<JetspeedPortletInstance>();
   ...
   */
  @Override
  public List<JetspeedPortletInstance> getInstances(RunData data)
      throws PortletException {
  ...
  protected void buildAllEntries(Portlets portlets,
      ArrayList<JetspeedPortletInstance> entries, Profile profile)
      throws PortletException {
