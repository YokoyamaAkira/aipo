...
/**
 * <p>
 * This is an implementation of the <code>JetspeedSecurityService</code>
 * interface.
 * 
 * 
 * @author <a href="mailto:david@bluesunrise.com">David Sean Taylor</a>
 * @author <a href="mailto:sgala@hisitech.com">Santiago Gala</a>
 * @version $Id: JetspeedDBSecurityService.java,v 1.25 2004/03/31 04:49:10
 *          morciuch Exp $
 */

public class JetspeedDBSecurityService extends TurbineBaseService implements
    JetspeedSecurityService {
  /**
   * Static initialization of the logger for this class
   */
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(JetspeedDBSecurityService.class.getName());
  ...
  private final static String[] DEFAULT_ADMIN_ROLES = { "admin" };
  ...
  private static HashMap<String, UserLogonStats> users =
    new HashMap<String, UserLogonStats>();
  ...
  public synchronized void init(ServletConfig conf)
    ...
    caseInsensitiveUsername =
      serviceConf.getBoolean(
        CONFIG_CASEINSENSITIVE_USERNAME,
        caseInsensitiveUsername);
    caseInsensitivePassword =
      serviceConf.getBoolean(
        CONFIG_CASEINSENSITIVE_PASSWORD,
        caseInsensitivePassword);
    caseInsensitiveUpper =
      serviceConf
        .getBoolean(CONFIG_CASEINSENSITIVE_UPPER, caseInsensitiveUpper);

    strikeCount = serviceConf.getInt(CONFIG_LOGON_STRIKE_COUNT, strikeCount);
    strikeInterval =
      serviceConf.getLong(CONFIG_LOGON_STRIKE_INTERVAL, strikeInterval);
    strikeMax = serviceConf.getInt(CONFIG_LOGON_STRIKE_MAX, strikeMax);

    autoLogonDisable =
      serviceConf.getBoolean(CONFIG_LOGON_AUTO_DISABLE, autoLogonDisable);
    actionsAnonDisable =
      serviceConf.getBoolean(CONFIG_ACTIONS_ANON_DISABLE, actionsAnonDisable);
    actionsAllUsersDisable =
      serviceConf.getBoolean(
        CONFIG_ACTIONS_ALLUSERS_DISABLE,
        actionsAllUsersDisable);
  ...
  /*
   * Factory to create a new JetspeedUser, using JetspeedUserFactory. The class
   * that is created by the default JetspeedUserFactory is configured in the
   ...
   * 
   * Given a username, converts the username to either lower or upper case. This
   * optional feature is configurable from the JetspeedSecurity.properties:
   * 
   * <code>services.JetspeedSecurity.caseinsensitive.username =
   * true/false</code> <code>services.JetspeedSecurity.caseinsensitive.upper =
   * true/false</code>
   * 
   * If <code>caseinsensitive.username</code> is true, then conversion is
   * enabled and the username will be converted before being sent to the
  ...
  public String convertUserName(String username) {
    if (caseInsensitiveUsername) {
      username =
        (caseInsensitiveUpper) ? username.toUpperCase() : username
          .toLowerCase();
    }
    return username;
  }
   ...
   * 
   * Given a password, converts the password to either lower or upper case. This
   * optional feature is configurable from the JetspeedSecurity.properties:
   * 
   * <code>services.JetspeedSecurity.caseinsensitive.password =
   * true/false</code> <code>services.JetspeedSecurity.caseinsensitive.upper =
   * true/false</code>
   * 
   * If <code>caseinsensitive.password</code> is true, then conversion is
   * enabled and the password will be converted before being sent to the
   * Authentication provider.
   * 
  ...
  public String convertPassword(String password) {
    if (caseInsensitivePassword) {
      password =
        (caseInsensitiveUpper) ? password.toUpperCase() : password
          .toLowerCase();
    }
    return password;
  }
   ...
   * 
   * Checks and tracks failed user-logon attempts. If the user fails to logon
   * after a configurable number of logon attempts, then the user's account will
   * be disabled.
   * 
   ...
   * 
   * The example setting below allows for 3 logon strikes per 300 seconds. When
   * the strike.count is exceeded over the strike.interval, the account is
   * disabled. The strike.max is the cumulative maximum.
   * 
   ...
   * 
   * These settings are not persisted, and in a distributed environment are only
   * tracked per node.
   * 
  ...
  public boolean checkDisableAccount(String username) {
    ...
    UserLogonStats stat = users.get(username);
    if (stat == null) {
  ...
  /**
   * Helper to UserManagement. Retrieves a <code>JetspeedUser</code> given the
   * primary principle username. The principal can be any valid Jetspeed
   * Security Principal:
  ...
  /**
   * Helper to PortalAuthorization. Gets a <code>JetspeedUser</code> from
   * rundata, authorize user to perform the secured action on the given
   * <code>Portlet</code> resource. If the user does not have sufficient
   * privilege to perform the action on the resource, the check returns false,
   * otherwise when sufficient privilege is present, checkPermission returns
   * true.
   * 
   * @param rundata
   *          request that the user is taken from rundatas
   * @param action
  ...
  public boolean checkPermission(JetspeedRunData runData, String action,
      Portlet portlet) {
    return JetspeedPortalAccessController.checkPermission(runData
      .getJetspeedUser(), portlet, action);
  }

  /**
   * Helper to PortalAuthorization. Gets a <code>JetspeedUser</code> from
   * rundata, authorize user to perform the secured action on the given
   * <code>Entry</code> resource. If the user does not have sufficient privilege
   * to perform the action on the resource, the check returns false, otherwise
   * when sufficient privilege is present, checkPermission returns true.
   * 
   * @param rundata
   *          request that the user is taken from rundatas
   * @param action
   *          the secured action to be performed on the resource by the user.
   * @param entry
   *          the portal entry resource.
   * @return boolean true if the user has sufficient privilege. public boolean
   *         checkPermission(JetspeedRunData runData, String action,
   *         RegistryEntry entry) { return
   *         JetspeedPortalAccessController.checkPermission
   *         (runData.getJetspeedUser(), entry, action); }
   */

  /*
   * Security configuration setting to disable all action buttons for the Anon
   * user This setting is readonly and is edited in the JetspeedSecurity
   * deployment
   * 
   * 
