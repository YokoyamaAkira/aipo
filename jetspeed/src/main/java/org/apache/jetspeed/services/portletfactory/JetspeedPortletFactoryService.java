 ...
 *          jford Exp $
 */
public class JetspeedPortletFactoryService extends TurbineBaseService implements
  ...
  public Portlet getPortlet(Entry entry) throws PortletException {
    ...
    if (regEntry == null) {
      throw new PortletException(
        "PortletFactory: unknown portlet entry in Registry: "
          + entry.getParent());
    }

    if (PortletEntry.TYPE_ABSTRACT.equals(regEntry.getType())) {
      throw new PortletException(
        "PortletFactory: can't instanciate abstract registry entry: "
          + regEntry.getName());
    }
  ...
  public Portlet getPortlet(String name, String id) throws PortletException {
    ...
    if (regEntry == null) {
      throw new PortletException(
        "PortletFactory: unknown portlet entry in Registry: " + name);
    }

    if (PortletEntry.TYPE_ABSTRACT.equals(regEntry.getType())) {
      throw new PortletException(
        "PortletFactory: can't instanciate abstract registry entry: " + name);
    }
   ...
   * @param pc
   *          the PortletConfig object to be associated with this object
   * @param id
   *          the PSML entry's portlet id
  ...
  /**
   * @param Entry
   *          entry Entry whose parent we want
  ...
  protected void addParentInitParameters(PortletEntry entry,
    ...
    PortletEntry parent = getParentEntry(entry);
    if (parent != null) {
      Map<String, String> parentMap = parent.getParameterMap();
