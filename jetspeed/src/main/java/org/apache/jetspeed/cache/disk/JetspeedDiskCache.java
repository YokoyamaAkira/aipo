...
public class JetspeedDiskCache implements DiskCache {
   ...
   */
  public static String DEFAULT_CACHE_DIRECTORY = JetspeedResources.getString(
    JetspeedResources.CACHE_DIRECTORY_KEY,
    "WEB-INF/cache");

  /**
   * Stores instances of JetspeedDiskCaches
   */
  private static Hashtable<String, JetspeedDiskCache> instances =
    new Hashtable<String, JetspeedDiskCache>();
   ...
   */
  private final Hashtable<String, JetspeedDiskCacheEntry> entries =
    new Hashtable<String, JetspeedDiskCacheEntry>();

  /**
   * Static initialization of the logger for this class
   */
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(JetspeedDiskCache.class.getName());
    ...
    if (DEFAULT_CACHE_DIRECTORY.equals("use-servlet-temp")) {
      ...
      try {
        ...
        if (logger.isDebugEnabled()) {
          logger
            .debug("DISK CACHE: will create cache in servlet temp directory "
              + tempdir);
        }
      } catch (Exception e) {
        logger
          .error("DISK CACHE: problems creating cache in servlet temp directory "
            + " falling back to WEB-INF/cache : "
            + e);
      }
      this.directory = tempdir;
    } else {
      if (logger.isDebugEnabled()) {
        logger.debug("DISK CACHE: will use cache in user configured directory "
          + directory);
      }
    }
  }
  ...
  private void initEntries() {
    ...
    // To whomever rewrite this code: please change API so the cache is
    // initialized
    // through the URLManager service *after* the Manager is initialized, and
    // possibly
    ...
    try {
      org.apache.turbine.services.TurbineServices
        .getInstance()
        .getService(
          org.apache.jetspeed.services.urlmanager.URLManagerService.SERVICE_NAME)
        .init();
    } catch (Throwable t) {
    ...
    if (files == null) {
      logger
        .error("DiskCache.initEntries: Error!!! - The cache directory cannot be found: "
          + directory);
    }
    ...
    for (int i = 0; i < files.length; ++i) {
      ...
      if (files[i].indexOf("http_") == 0) {
        ...
        JetspeedDiskCacheEntry ent =
          new JetspeedDiskCacheEntry(new File(getRoot(), files[i]));
  ...
  public DiskCacheEntry[] getEntries() {
    ...
    Vector<JetspeedDiskCacheEntry> diskEntries =
      new Vector<JetspeedDiskCacheEntry>();
  ...
  public DiskCacheEntry getEntry(String url, boolean force) throws IOException {
    ...
    if (url == null) {
      throw new IllegalArgumentException(
        "You must specify a URL to obtain an entry from the cache");
    }

    // return right away if the entry exists in the cache...
    String interned = url.intern();
    JetspeedDiskCacheEntry entry = entries.get(interned);
    if (entry != null) {
    ...
    if (DiskCacheUtils.isLocal(url)) {
      ...
      JetspeedDiskCacheEntry dce = entries.get(local);
      if (dce == null) {
    ...
    if (DiskCacheUtils.isCacheable(url)) {
      ...
      logger
        .info("DiskCache: this URL can't be stored in cache... providing it directly."
          + url);
  ...
  public DiskCacheEntry getEntry(String url, Reader is) throws IOException {
    ...
    JetspeedDiskCacheEntry dce = entries.get(url.intern());
    if (dce != null) {
      dce.setFile(file);
      return dce;
    } else {
   ...
   *          what file to store it in.
   */
  public String fetch(String url, String cache, boolean force)
      throws IOException {
    ...
    try {
      ...
      OutputStreamWriter os =
        new OutputStreamWriter(new FileOutputStream(cache), "utf-8");
   ...
   * 
   * @param location
   *          A directory to store the cache at.
   */
