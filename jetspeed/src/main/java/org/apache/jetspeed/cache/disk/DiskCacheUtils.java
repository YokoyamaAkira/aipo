 ...
 * <ul>
 * <li>
 * Virtual: URLs which are contructored such as: /test/test.xml</li>
 * 
 * <li>
 * Local: URLs which are contructored such as: http://localhost/test/test.xml</li>
 * 
 * <li>
 * Remote: URLs which are contructored such as:
 * http://REMOTE.SERVER/test/test.xml</li>
 * 
 * </ul>
 * </p>
 * 
...
public class DiskCacheUtils {
   ...
   */
  private static Vector<String> localprotocols = JetspeedResources
    .getVector("diskcache.localprotocols");

  /**
   * Static initialization of the logger for this class
   */
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(DiskCacheUtils.class.getName());
  ...
  public static String getLocalURL(String virtual) {
    ...
    if (virtual != null && isLocal(virtual) && virtual.indexOf("/") != 0) {
      return virtual;
    }

    if (isVirtual(virtual) == false) {
      throw new IllegalArgumentException(
        "The URL specifies is not a virtual URL: " + virtual);
    }
  ...
  public static boolean isLocal(String url) {
    ...
    if (url != null && url.length() > 1) {
       ...
       * 
       * EX: http://localhost http://server.domain
       */
      ...
      if (localprotocols != null) {
        ...
        while (en.hasMoreElements()) {
          String protocol = en.nextElement() + ":";
  ...
  public static boolean isCached(DiskCache instance, String url) {
    ...
    /*
     * if ( isLocal( url ) ) { //SGP: I think we should not cache local urls
     * return false; }
     */
  ...
  /**
   * Given a url and an disk cache instance, determine what the correct URL for
   * this cache entry for the remote URL would be.
   */
