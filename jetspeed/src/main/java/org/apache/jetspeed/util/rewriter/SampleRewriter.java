...
public class SampleRewriter extends HTMLRewriter {
  ...
  private final String fullPortalURL;
   ...
   */
  public SampleRewriter(String basePortalURL, String fullPortalURL,
      String sampleURL) {
  ...
  /*
   * <p> This callback is called by the HTMLParserAdaptor implementation to
   * write back all rewritten URLs to point to the proxy server. The MMS
   * implementation writes specifically for network element ids and relative
   * paths to MMS resources. </p> <p> Given the targetURL, rewrites the link as
   * a link back to the proxy server. </p>
   * 
   * Example format:
   * 
  ...
  public String generateNewUrl(String targetURL, HTML.Tag tag,
    ...
    if (debug) {
      System.out.println("[rewriter] Tag: "
        + tag.toString()
        + "  Attribute: "
        + attribute.toString()
        + "  targetURL: "
    ...
    if (tag == HTML.Tag.FORM && attribute == HTML.Attribute.ACTION) {
      ...
      if (sampleEndFlag) {
        // The sample session is being terminated, make the form action return
