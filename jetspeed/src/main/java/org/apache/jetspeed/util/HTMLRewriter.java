...
public class HTMLRewriter {
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(HTMLRewriter.class.getName());

  private final HTMLRewriter.Callback cb = new HTMLRewriter.Callback();

  /**
   * Sets the parameters for the HTMLRewriter
   * 
   * @param removeScript
   *          Shall SCRIPT-Tags and their content be removed
   * @param removeStyle
   *          Shall STYLE-Tags and their content be removed
   * @param removeNoScript
   *          Shall NOSCRIPT-Tags and their content be removed
   * @param removeMeta
   *          Shall META-Tags be removed
   * @param removeApplet
   *          Shall APPLET-Tags and their content be removed
   * @param removeObject
   *          Shall OBJECT-Tags and their content be removed
   * @param removeHead
   *          Shall HEAD-Tags and their content be removed
   * @param removeOnSomething
   *          Shall onClick, onBlur, etc. -Attributes be removed
   */
  public HTMLRewriter(boolean removeScript, boolean removeStyle,
      boolean removeNoScript, boolean removeMeta, boolean removeApplet,
      boolean removeObject, boolean removeHead, boolean removeOnSomething) {
    init(
      removeScript,
      removeStyle,
      removeNoScript,
   ...
   *          Shall links set Target="_blank"
   */
  private void init(boolean removeScript, boolean removeStyle,
      boolean removeNoScript, boolean removeMeta, boolean removeApplet,
      boolean removeObject, boolean removeHead, boolean removeOnSomething,
      boolean openInNewWindow) {
   ...
   * 
   * @param HTMLrdr
   *          Reader for HTML to be converted
   * @param BaseUrl
   *          URL from which this HTML was taken. We be the base-Url for all
   *          URL-rewritings.
   * @throws MalformedURLException
   *           If the BaseUrl is not a valid URL or if an URL inside the
   *           document could not be converted. Should not happen normally, even
   *           in badly formatted HTML.
   * @return HTML-String with rewritten URLs and removed (according to
   *         constructor-settings) tags
   */
  public synchronized String convertURLs(Reader HTMLrdr, String BaseUrl)
      throws MalformedURLException {
  ...
  /**
   * That Class is needed, because getParser is protected and therefore only
  ...
  class Callback extends HTMLEditorKit.ParserCallback {
    ...
    /**
     * Because Scripts and Stlyle sometimes are defined in comments, thoese will
     * be written. Otherwise comments are removed
     */
    @Override
    ...
    public void handleEndTag(HTML.Tag tag, int position) {
      ...
      if (removeScript && (tag == HTML.Tag.SCRIPT)) {
        ...
        ignoreLevel--;
      } else if (removeNoScript
        && (tag.toString().equalsIgnoreCase("NOSCRIPT"))) {
        ignoreLevel--;
      }
    }
    ...
    private void appendTagToResult(HTML.Tag tag, MutableAttributeSet attrs) {
      ...
      while (e.hasMoreElements()) {
        ...
        // with "on")
        if (!(removeOnSomething && attrName.toLowerCase().startsWith("on") && (attrName
          .length() > 2))) {
          // Attribute included
          addToResult(" ").addToResult(attr).addToResult("=\"").addToResult(
            value).addToResult("\"");
        }
      }
      addToResult(">");
     ...
     * 
     * @param tag
     *          TAG from the Callback-Interface
     * @param attrs
     *          Attribute-Set from the Callback-Interface
     */
    ...
    private void convertURLS(HTML.Tag tag, MutableAttributeSet attrs) {
      ...
      if (tag == HTML.Tag.A) {
        if (attrs.getAttribute(HTML.Attribute.HREF) != null) {
          // ---- CHECKING <A HREF
          addConvertedAttribute(HTML.Attribute.HREF, attrs);
        }
        if ((attrs.getAttribute(HTML.Attribute.TARGET) == null)
          && cb.openInNewWindow) {
        ...
        addConvertedAttribute(HTML.Attribute.SRC, attrs);
        ...
        if (attrs.getAttribute(HTML.Attribute.CODEBASE) == null) {
          ...
          attrs.addAttribute(HTML.Attribute.CODEBASE, baseUrl
            .toString()
            .substring(0, endOfPath + 1));
        } else {
        ...
        if (attrs.getAttribute(HTML.Attribute.HREF) != null) {
          try {
            baseUrl =
              new URL(attrs.getAttribute(HTML.Attribute.HREF).toString());
          } catch (Throwable t) {
        ...
        if (attrs.getAttribute(HTML.Attribute.ACTION) == null) {
          ...
          attrs.addAttribute(HTML.Attribute.ACTION, baseUrl.toString());
        } else {
          addConvertedAttribute(HTML.Attribute.ACTION, attrs);
        }
      } else if (tag == HTML.Tag.TD) {
        // ---- CHECKING <TD BACKGROUND=
        if (!(attrs.getAttribute(HTML.Attribute.BACKGROUND) == null)) {
          addConvertedAttribute(HTML.Attribute.BACKGROUND, attrs);
        }
      }
    ...
    private void addConvertedAttribute(HTML.Attribute attr,
      ...
      if (attrs.getAttribute(attr) != null) {
        ...
        attrs.addAttribute(attr, generateNewUrl(attrSource));
      }
    }
