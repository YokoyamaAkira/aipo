...
public class ClipperRewriter extends HTMLRewriter {
  ...
  private final String NOT_FOUND = "<br>Element not found, returning null<br>";

  private final String INVALID_START =
    "<br>Error: received null start element<br>";
  ...
  public String rewrite(Reader input, String baseURL)
    ...
    if (Log.getLogger().isDebugEnabled()) {
      ...
      Log.debug("Clipper rewriter: foundElementNumber:" + foundElementNumber);
  ...
  public boolean enterText(char[] values, int param) {
    if ((foundStart == true) && (foundStop == false) && (stopElement != null)) {
      return true;
    } else {
      return false;
    }
   ...
   * 
   * @param startElementNumber
   *          the new start element number
   */
  ...
  /*
   * Control if searched tags are coupled. If searched tags are coupled, we must
   * consider nested tags, example: Searched: <table> and </table> in the page
   * there is: <table>... <table>... </table> </table> We are searching for the
   * first and fourth tag!
   */
  ...
  private boolean compareTag(String tag, String attrs, String base) {
    ...
    while (baseTok.hasMoreTokens()) {
      ...
      // Exact match for the tag, for the attrs it's simpler to control the
      // index
      if (token.equals(tag)) {
        foundTag = true;
      } else if (attrs == null) {
    ...
    if (foundTag == false) {
      ...
      if (Log.getLogger().isDebugEnabled()) {
        Log.debug("Clipper rewriter: match between tag "
          + tag
          + ", attrs "
          + attrs
          + ", and searched: "
