...
/**
 * A customized version of the DynamicURI for linking to non-servlet webapp
 * resources.
 * 
...
public class ContentTemplateLink extends DynamicURI implements ApplicationTool {
  ...
  /**
   * decide whether we should output full external URIs or simple absolute URIs
   */
  ...
  /**
   * This will initialise a ContentTemplateLink object that was constructed with
   * the default constructor (ApplicationTool method).
   * 
   * @param data
  ...
  protected void initContextPath(Object data) {
    try {
      ...
      Method meth =
        runDataClass.getDeclaredMethod("getContextPath", (Class<?>[]) null);
      contextPath = (String) meth.invoke(data, (Object[]) null);
    } catch (Exception e) {
      /*
       * Ignore a NoSuchMethodException because it means we are using Servlet
       * API 2.0. Make sure scriptName is not null.
       */
      contextPath = "";
    }
  }
  ...
  /**
   * Specify the link should be expressed in external form (ie with protocol,
  ...
  /**
   * Specify the link should be expressed in absolute form (ie only a URI and
   * not a full URL)
   * 
   * @return a self reference for easy link construction in templates
   */
   ...
   * 
   * @param pathToContent
   *          the path to resource, assumed to be relative to the web
   *          application context
   * @return a self reference for easy link construction in templates
   */
  ...
  /**
   * Returns the URI. After rendering the URI, it clears the pathInfo and
   * QueryString portions of the DynamicURI.
   * 
