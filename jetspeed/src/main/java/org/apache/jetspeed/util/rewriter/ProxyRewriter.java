 ...
 * URLs or as web-application full URLs, not relative. Given a relative URL,
 * such a "/content/images/my.gif" it can be rewritten as either a proxied URL,
 * for example:
 * 
...
public class ProxyRewriter implements Rewriter {
   ...
   * stream, rewriting relevant links, and writes the parsed stream back to the
   * client.
   * 
   ...
   */
  public String rewrite(Reader input, String proxyRoot, String baseURL)
   ...
   * back all rewritten URLs to point to the proxy server. Given the targetURL,
   ...
   */
  @Override
  public String generateNewUrl(String targetURL, HTML.Tag tag,
  ...
  public static byte[] rewriteScript(String script, String url,
      String proxyHost, String base) throws IOException {
    ...
    for (int ix = 0; ix < chars.length; ix++) {
      if (chars[ix] == '"') {
        ...
        // endpos < totalScriptLength)
        {
          ...
          writer.write(trans);
          if (chars[ix + 1] != PATH_SEPARATOR
            && base.charAt(baseLength - 1) != PATH_SEPARATOR) {
  ...
  /*
   * Finds an image hyperlink in a quoted string. The image hyperlink is found
   * by searching through the script text, searching for references ending in
   * typical image extensions (GIF, PNG, JPG).
   * 
   * NOTE: this function is just the start of script-parsing. A much more robust
