...
/**
 * This class is used to either strip and/or insert PIs in a XML document. It
 * uses SAX-1 API and outputs text to an output stream. WARNING: This behavior
...
public class SAXPIFilter extends HandlerBase {
  ...
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(SAXPIFilter.class.getName());
  ...
  private final boolean stripExistingPI;

  private final Vector<String> pis = new Vector<String>();

  /**
   * Creates of simple parser which outputs its document to the PrintWriter
   * passed as arguments. In this mode the parser may be used as a simple
   * well-formedness checker.
   * 
   * @param outPW
   *          the printWriter where to output parsed data
   */
  ...
  /**
   * In this mode the parser may be used as a simple well-formedness checker or
   * a PI stripper.
   * 
   * @param outPW
   *          the printWriter where to output parsed data
   * @param strip
   *          configure the parser to strip PIs if strip is true
   */
  public SAXPIFilter(PrintWriter outPW, boolean strip)
      throws UnsupportedEncodingException {
    this(outPW, strip, null);
  }

  /**
   * In this mode the parser can strip existing PIs and insert new ones just
   * after the document declaration
   * 
   * @param outPW
   *          the printWriter where to output parsed data
   * @param strip
   *          configure the parser to strip PIs if strip is true
   * @param PI
   *          string reprensenting the PI to be output after the document
   *          declaration
   */
  public SAXPIFilter(PrintWriter outPW, boolean strip, String PI)
      throws UnsupportedEncodingException {
    this.out = outPW;
   ...
   * 
   * @param uri
   *          URL where to fetch the document to be parsed
   */
