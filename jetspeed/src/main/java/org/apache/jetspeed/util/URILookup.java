 ...
 * 
 * <p>
 * Functions to get an URI based on a type like info, edit, save and to identify
 * the type of an URI.
 * </p>
 * 
 * <p>
 * To overwrite the default behaviour, specify one of the following parameters
 * in the JR.p file. URILookup will then return the uri specified in the
 * properties file.
 * </p>
 * <p>
...
public class URILookup {
   ...
   *          subtype of the URI
   * @param aPortlet
   *          Portlet the URI points to
   * @param rundata
   *          the RunData object
   * @return the URI
   */
  public static String getURI(int aType, int aSubType, Portlet aPortlet,
   ...
   * @return the URI
   */
  public static String getURI(int aType, int aSubType, String aPortletName,
      RunData rundata) throws JetspeedException {
   ...
   *          subtype of the URI
   * @param userData
   *          string which should be added to the URL
   * @param aPortlet
   *          Portlet the URI points to
   * @param rundata
   *          the RunData object
   * @return the URI
   */
  public static String getURI(int aType, int aSubType, String userData,
      Portlet aPortlet, RunData rundata) throws JetspeedException {
    if (aPortlet != null) {
   ...
   */
  public static String getURI(int aType, int aSubType, String userData,
      String aPortletName, RunData rundata) throws JetspeedException {
    String newURI = null;
    ...
    String propertiesParameterValue =
      JetspeedResources.getString(propertiesParameter, null);

    if (propertiesParameterValue != null) {
      // found the parameter value, so replace the newURI with this one
      if (logger.isInfoEnabled()) {
        logger.info("URILookup: replaced uri "
          + newURI
          + " with "
          + propertiesParameterValue);
      }
    ...
    // remove starting slash, so that the URI is relative, and the Base-Tag is
    // used
    // Note: if there is no starting slash, the function encodeURL inserts one
    // slash in front of the URI
  ...
  /**
   * Gets the type of the URI (e.g. TYPE_INFO, TYPE_EDIT). The default return
   * value is TYPE_HOME
   * <P>
   * Hint:<BR>
   * Portlets should check for TYPE_EDIT_PORTLET and in any other case render
   * the content
   * </P>
   * 
   * @param aPortlet
   *          the associated portlet
   * @param rundata
   *          the RunData object
   * @return the type
   */
  public static int getURIType(Portlet aPortlet, RunData rundata) {
    if (aPortlet != null) {
      ...
      if (aPortlet.getName().equals(((JetspeedRunData) rundata).getPortlet())) {
        if (rundata.getScreen() != null) {
          if (rundata.getScreen().equals(SCREEN_INFO)) {
            return TYPE_INFO;
          }
        }
      }
    ...
    if (rundata.getScreen() != null) {
      ...
      } else if (rundata.getScreen().equals(SCREEN_NEWACCOUNT)) {
        return TYPE_ENROLLMENT;
      } else if (rundata.getScreen().equals(SCREEN_EDIT_ACCOUNT)) {
        return TYPE_EDIT_ACCOUNT;
      } else if (rundata.getScreen().equals(SCREEN_LOGIN)) {
        return TYPE_LOGIN;
      }
    }
  ...
  public static int getURISubType(Portlet aPortlet, RunData rundata)
    ...
    if (rundata.getAction() != null) {
      if (rundata.getAction().equals(ACTION_ACCEPT_LOGIN)) {
        return SUBTYPE_ACCEPT_LOGIN;
      } else if (rundata.getAction().equals(ACTION_LOGOUT)) {
        return SUBTYPE_LOGOUT;
      }
    }
  ...
  public static String getWebAppBaseDirURI(RunData rundata) {
    String ctxtPath =
      JetspeedResources.getString(JetspeedResources.CONTENT_ROOT_URL_KEY, "");
  ...
  /**
   * <p>
   * Given a ParameterParser, get a PortletEntry. This is used so that when you
   * have a URI created from PortletURIManager you can get back the PortletEntry
   ...
   * @return the new URI
   */
  private static String replaceTurbineURIParameter(String uri,
   ...
   */
  @SuppressWarnings("unused")
  private static String addURIParameter(String uri, String parameter,
      String value) {
   ...
   *          the URI to be modified
   * @param data
   *          the data to be added (has to be the correct format -
   *          paramater=value)
   * @return the modified URI
   */
  ...
  private static final String SCREEN_HOME = JetspeedResources
    .getString("template.homepage");

  // private static final String SCREEN_CUSTOMIZE = "Customize";
  private static final String SCREEN_CUSTOMIZE = JetspeedResources
    .getString("customizer.screen");

  private static final String SCREEN_LOGIN = JetspeedResources
  ...
  private static final String ACTION_CUSTOMIZER = JetspeedResources
    .getString("customizer.action");

  private static final String ACTION_MARKPAGE = "MarkRefPage";

  private static final String ACTION_LOGOUT = JetspeedResources
    .getString("action.logout");

  private static final String ACTION_ACCEPT_LOGIN = JetspeedResources
