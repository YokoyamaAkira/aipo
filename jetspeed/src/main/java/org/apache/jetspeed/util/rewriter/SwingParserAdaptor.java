...
public class SwingParserAdaptor implements HTMLParserAdaptor {
  ...
  private final SwingParserAdaptor.Callback cb =
    new SwingParserAdaptor.Callback();

  private final String lineSeparator;

  @SuppressWarnings("unused")
  private final boolean skippingImplied = false;

  private final Rewriter rewriter;

  /*
   * Construct a swing (hot java) parser adaptor Receives a Rewriter parameter,
   * which is used as a callback when rewriting URLs. The rewriter object
   * executes the implementation specific URL rewriting.
   * 
   * @param rewriter The rewriter object that is called back during URL
   * rewriting
   */
  ...
  /*
   * Parses and an HTML document, rewriting all URLs as determined by the
   * Rewriter callback
   * 
   * 
  ...
  public String run(Reader reader) throws MalformedURLException {
    HTMLEditorKit.Parser parser =
      new SwingParserAdaptor.ParserGetter().getParser();
  ...
  /*
   * This Class is needed, because getParser is protected and therefore only
   * accessibly by a subclass
   */
  ...
  /*
   * Swing Parser Callback from the HTMLEditorKit. This class handles all
   * SAX-like events during parsing.
   */
  class Callback extends HTMLEditorKit.ParserCallback {
    ...
    @SuppressWarnings("unused")
    private final boolean inScript = false;
     ...
     */
    @Override
    public void handleStartTag(HTML.Tag tag, MutableAttributeSet attrs,
        int position) {
    ...
    boolean isValidFragmentTag(HTML.Tag tag) {
       ...
       * 
       * if (tag == HTML.Tag.FRAMESET) // always strip out FRAMESET tag for
       * fragments return false;
       * 
       * if (tag == HTML.Tag.FRAME) return false;
       * 
       * if (tag == HTML.Tag.NOFRAMES) return false;
       */
      return true;
    }
    ...
    /*
     * Used to write tag and attribute objects to the output stream. Returns a
     * reference to itself so that these calls can be chained.
     * 
     * @param txt Any text to be written out to stream with toString method. The
     * object being written should implement its toString method.
     * 
    ...
    /*
     * Used to write all character content to the output stream. Returns a
     * reference to itself so that these calls can be chained.
     * 
     ...
     * 
     * TODO: it would be better to drive this logic off a state table that is
     * not tied to the Hot Java parser.
     * 
    ...
    private void convertURLS(HTML.Tag tag, MutableAttributeSet attrs) {
      ...
      if ((tag == HTML.Tag.A)
      ...
      } else if (((tag == HTML.Tag.IMG || tag == HTML.Tag.INPUT) && (attrs
        .getAttribute(HTML.Attribute.SRC) != null))) {

        // ---- CHECKING <IMG SRC & <INPUT SRC
        addConvertedAttribute(tag, HTML.Attribute.SRC, attrs, rewriter
          .proxyAllTags());
        ...
        // ---- CHECKING <LINK
        addConvertedAttribute(tag, HTML.Attribute.HREF, attrs, rewriter
          .proxyAllTags());

      } else if (tag == HTML.Tag.APPLET) {

        // ---- CHECKING <APPLET CODEBASE=
        addConvertedAttribute(tag, HTML.Attribute.CODEBASE, attrs, rewriter
          .proxyAllTags());
        ...
        // ---- CHECKING <BODY
        addConvertedAttribute(tag, HTML.Attribute.BACKGROUND, attrs, rewriter
          .proxyAllTags());
      ...
      /*
       * if ( removeScript && (tag == HTML.Tag.SCRIPT)) { ignoreLevel ++;
       */
    }
     ...
     * as a proxied URL, or as a fullpath to the original host.
     * 
     ...
     * otherwise it is written back as a fullpath back to the original host.
     */
    private void addConvertedAttribute(HTML.Tag tag, HTML.Attribute attr,
        MutableAttributeSet attrs, boolean proxy) {
      if (proxy) {
        addProxiedConvertedAttribute(tag, attr, attrs);
      } else {
        if (attrs.getAttribute(attr) != null) {
          attrs.addAttribute(attr, generateNewUrl(tag, attrs, attr, false));
        }
      }
    }
     ...
     * 
     * @param attr
     *          The HTML attribute to be proxied.
     * @param attrs
     *          The mutable HTML attribute set for the current HTML element.
     * 
     */
    private void addProxiedConvertedAttribute(HTML.Tag tag,
        HTML.Attribute attr, MutableAttributeSet attrs) {
      ...
      if (attrs.getAttribute(attr) != null) {
        ...
        if (attrSource.startsWith("mailto:")) {
          attrs.addAttribute(attr, generateNewUrl(tag, attrs, attr, true));
     ...
     * url into a new fullpath URL, either relative to the proxy server, or a
     * fullpath to the original web server, depending on the 'proxied'
     * parameter.
     * 
     * @param oldURL The original URL, before it is tranlated.
     * 
     * @param proxied Boolean indicator denotes if the URL should be written
     * back as a proxied URL (true), or as a fully addressable address to the
     * original web server.
     * 
     * @return The translated new URL.
     */
    private String generateNewUrl(HTML.Tag tag, MutableAttributeSet attrs,
        HTML.Attribute attr, boolean proxied) {
