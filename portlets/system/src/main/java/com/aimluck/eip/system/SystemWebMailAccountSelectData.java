 ...
 */
public class SystemWebMailAccountSelectData extends
    ALAbstractSelectData<EipMMailAccount, EipMMailAccount> {
  /** logger */
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(SystemWebMailAccountSelectData.class.getName());
    ...
    isAdmin =
      rundata.getScreen().equals(
        ...
        || action.getClass().getName().equals(
          SystemManagerAction.class.getName());

    unreadMailSumMap =
      SystemWebMailUtils.getUnreadMailNumberMap(rundata, userId, accountId);
    mailFolderList =
      SystemWebMailUtils.getMailFolderAll(ALMailUtils.getMailAccount(
        userId,
        accountId));
  ...
  protected EipMMailAccount selectDetail(RunData rundata, Context context) {
    ...
    if (isAdmin) {
      ...
      int accountId =
        rundata.getParameters().getInt(SystemWebMailUtils.ACCOUNT_ID, -1);
  ...
  protected Object getResultDataDetail(EipMMailAccount record) {
    try {
      SystemWebMailAccountDetailResultData rd =
        new SystemWebMailAccountDetailResultData();
      rd.initField();
      rd.setAccountId(record.getAccountId().intValue());
      rd.setAccountName(record.getAccountName());
