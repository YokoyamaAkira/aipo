...
/**
 * タイムカードをJSONデータとして出力するクラスです。 
 * 
 */
public class TimecardFormJSONScreen extends ALJSONScreen {

  /** logger */
  private static final JetspeedLogger logger = JetspeedLogFactoryService
    .getLogger(TimecardFormJSONScreen.class.getName());
  ...
  protected String getJSONString(RunData rundata, Context context)
    ...
    try {
      ...
      if (ALEipConstants.MODE_INSERT.equals(mode)) {
        ...
        TimecardFormData formData = new TimecardFormData();
        formData.initField();
        formData.setWorkFlag(TimecardUtils.WORK_FLG_ON);
        if (formData.doInsert(this, rundata, context)) {
        } else {
          JSONArray json =
            JSONArray
      ...
      } else if ("punchout".equals(mode)) {
        TimecardFormData formData = new TimecardFormData();
        formData.initField();
        formData.setWorkFlag(TimecardUtils.WORK_FLG_OFF);
        if (formData.doInsert(this, rundata, context)) {
        } else {
          JSONArray json =
            JSONArray
      ...
      logger.error("[TimecardFormJSONScreen]", e);
    }
