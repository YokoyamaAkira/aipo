...
if(aipo.userAgent.isAndroid()){A.query("input,select,button").forEach(function(B,C){B.disabled=true
})
}},_closeDropDown:function(){aimluck.widget.Dropdown.prototype._closeDropDown.apply(this);
if(aipo.userAgent.isAndroid()){A.query("input,select,button:not(.disabled)").forEach(function(B,C){B.disabled=false
})
}},postCreate:function(){this.inherited(arguments);
var C={widgetId:this.listWidgetId,dateId:this.dateId,callback:this.callback};
this.dropDown=new aipo.widget.DateCalendar(C,this.listWidgetId);
if(this.initValue!=""){var D=this.initValue.split("/");
if(D.length==3){var F=D[0];
var B=D[1]-1;
var E=D[2];
var G=A.byId(this.dateId);
G.value=this.initValue;
this.dropDown.clearDate();
this.dropDown.setValue(new Date(F,B,E))
